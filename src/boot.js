...
(() => {
 ...
 // ---- Globals populated by boot + background ----
  ...
  function once(fn) {
    let done = false, val;
    return (...args) => { if (done) return val; done = true; val = fn(...args); return val; };
  }
  ...
  async function ensureHudLoaded() {
    if (typeof window.__VG_INIT_HUD__ === "function" &&
        typeof window.__VG_PLACE_HUD__ === "function") return;
    try {
   ...
   function killLegacyPill() {
    ...
    try {
      ...
      if (window.__VG_LEGACY_INTERVAL__) clearInterval(window.__VG_LEGACY_INTERVAL__);
      if (window.__VG_LEGACY_OBSERVER__?.disconnect) window.__VG_LEGACY_OBSERVER__.disconnect();
...
function __vgTeardownHUD() {
  try {
    ...
    if (frame) {
      ...
      try { frame.__VG_STICKY_CLEANUP__?.(); } catch {}
  ...
  try { window.__VG_DISABLED_BY_USER = true; } catch {}
}

async function __vgMountHUD() {
  try { window.__VG_DISABLED_BY_USER = false; } catch {}
...
async function fetchAndApplyPlacement() {
  ...
  try {
    if ((/(^|\.)v0\.app$/i).test(location.hostname) && window.top && window.top !== window) {
      host = String(window.top.location.hostname).toLowerCase().replace(/^www\./, "");
  ...
  try {
    const ask = () => new Promise((res) =>
      chrome.runtime.sendMessage({ type: "VG_GET_PAGE_PLACEMENTS", host, path }, res)
    );
    ...
    rules = (resp && resp.ok && Array.isArray(resp.placements)) ? resp.placements : [];
  } catch (e) {
  ...
  try {
    ...
    console.debug('[CT][INIT_RULES]', {
      ...
      first_dx: first?.dx, first_dy: first?.dy,
  ...
  const filtered = rules.filter(r => {
    ...
    const h = String(r.host || "").toLowerCase().replace(/^www\./, "").trim();
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    ...
    setTimeout(() => {
      if (!document.getElementById('__vg_iframe_hud__')) {
        ...
        try { window.__VG_DB_PLACEMENT = null; } catch {}
        fetchAndApplyPlacement()?.catch(() => {});
        try {
          const host = location.hostname.toLowerCase().replace(/^www\./,'');
          const path = location.pathname || '/';
          chrome.runtime.sendMessage({ type: 'VB_PLACEMENT_SUB', host, path }, () => {});
        } catch {}
      }
    }, 450);
  });
...
try {
  chrome.runtime.onMessage.addListener((msg) => {
    ...
    if ((msg.host || '').toLowerCase().replace(/^www\./,'') !== hostNow) return;
...
function __vgInstallRouteWatcher(onChange) {
  ...
  const fire = () => {
    ...
    if (now === last) return;
    last = now;
    try { window.dispatchEvent(new CustomEvent('VG_ROUTE_CHANGE', { detail: { href: now } })); } catch {}
    try { onChange?.(); } catch {}
  };
...
if (frame) {
  ...
  window.addEventListener('VG_ROUTE_CHANGE', __vgOnRouteChange, { passive: true });
})();
