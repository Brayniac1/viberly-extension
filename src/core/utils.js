// ==== utils.js — direct Supabase auth (no background messaging) ====
...
function authMsg(text, type='err'){ const m=$('authMsg'); if(!m) return;
  m.textContent = text||''; m.classList.remove('ok','err'); m.classList.add(type==='ok'?'ok':'err'); m.hidden = !text; }
function authMsgClear(){ const m=$('authMsg'); if(m) m.hidden = true; }
...
async function getBillingSummary() {
  const { data: { session } } = await db.auth.getSession();
  if (!session?.user) return { tier: 'free', used: 0, limit: PLAN_LIMITS.free, status: 'inactive' };
...
async function startCheckout(tier /* 'basic' | 'pro' */) {
  const price_id =
    tier === 'basic' ? 'price_1RyYJuCKsHaxtGkUiLlRAAd3' :
    tier === 'pro'   ? 'price_1RyYMaCKsHaxtGkUMaScJLZS' : null;

  if (!price_id) { console.warn('[checkout] unknown tier', tier); return; }

  const { data: { session } } = await db.auth.getSession();
  if (!session?.access_token) { console.warn('[checkout] no session'); return; }
...
async function openPortal() {
  ...
  if (!r.ok) { console.error('[portal] failed'); return; }
  const { url } = await r.json();
  try { chrome.tabs.create({ url }); } catch { window.open(url, '_blank'); }
}


	// ---- Enhanced session hydration with validation ----
	(async () => {
	  try {
	    // If Supabase already has the session in memory, validate it first
	    ...
	    if (session?.access_token && session?.expires_at) {
	      ...
	      if (expiresAt && (expiresAt - now) < 120) {
	        console.log('[popup] Token expiring soon, attempting refresh...');
	        try {
	          const { data: refreshData, error } = await db.auth.refreshSession();
	          if (!error && refreshData?.session) {
	            session = refreshData.session;
	            console.log('[popup] Token refreshed successfully');
	          }
	        } catch (e) {
	          console.warn('[popup] Token refresh failed:', e);
	        }
	      }
	    }
	        ...
	        try {
	          ...
	          if (sbKey) {
	            ...
	            if (cs?.access_token && cs?.refresh_token) {
	              ...
	              const { data: { session: restoredSession } } = await db.auth.getSession();
	              if (restoredSession?.expires_at) {
	                const now = Math.floor(Date.now() / 1000);
	                const expiresAt = restoredSession.expires_at;
	                if (expiresAt && (expiresAt - now) < 120) {
	                  console.log('[popup] Restored token expiring soon, refreshing...');
	                  try {
	                    await db.auth.refreshSession();
	                  } catch (e) {
	                    console.warn('[popup] Restored token refresh failed:', e);
	                  }
	                }
	              }
	            }
...
async function renderAuthState() {
  try {
    ...
    if (session.user) {
      ...
      const label = $('acctEmail'); if (label) label.textContent = email;

      const { tier, used, limit } = await getBillingSummary();
      const planEl = $('acctPlan'); if (planEl) planEl.textContent = tier === 'pro' ? 'Pro' : tier === 'basic' ? 'Basic' : 'Free';
      const usageEl = $('acctUsage'); if (usageEl) usageEl.textContent = (limit === Infinity) ? `${used} / ∞` : `${used} / ${limit}`;
    } else {
...
btnLogin?.addEventListener('click', async () => {
  ...
  try {
    ...
    await db.from('vg_profiles').upsert(
...
linkForgot?.addEventListener('click', async (ev) => {
  ...
  try {
    ...
    const redirectTo = chrome?.identity?.getRedirectURL
      ? chrome.identity.getRedirectURL('reset-finish')
      : undefined;
