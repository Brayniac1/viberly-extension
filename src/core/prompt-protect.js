...
(() => {
  ...
  function badge(el, msg = "Protected prompt") {
    try {
  ...
  PROTECT.insertPaidPrompt = function (
    targetEl,
    { promptId, description, vars }
  ) {
    if (!isEditable(targetEl)) return;
    browser.runtime
      .sendMessage({ type: "VG_COMPOSE", promptId, vars })
      .then((resp = {}) => {
        const { ok, messageUid, err } = resp;
        if (!ok || !messageUid) {
          console.warn("VG_COMPOSE failed", err);
          return;
        }
        ...
        PENDING.set(messageUid, {
          targetElRef: new WeakRef(targetEl),
          at: Date.now(),
        });
        badge(targetEl);
      });
  ...
  PROTECT.prepareSend = function (editorEl, strategy = "swap") {
    try {
      ...
      return new Promise((resolve) => {
        browser.runtime
          .sendMessage({ type: "VG_GET_REAL", messageUid })
          .then((resp = {}) => {
  ...
  PROTECT.wireSendGuards = function ({
    editorEl,
    sendBtn,
    strategy = "swap",
  } = {}) {
