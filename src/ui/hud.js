  ...
  if (!window.__VG_PILL_CLICK_WIRED__) {
    ...
    (function initVgInsertImageOnce() {
      ...
      window.vgInsertImage = async function vgInsertImage(
        ...
        try {
          // Detect ChatGPT and Claude AI to prevent dual insertion
          const IS_CHATGPT =
          ...
          // Only use fallback link for non-ChatGPT/Claude sites
          if (!ok && !IS_CHATGPT && !IS_CLAUDE) {
            fallbackLink(target || document.body, dataUrl);
          }
