...
if (!window.__VG_PILL_CLICK_WIRED__) {
  ...
  (function initVgInsertImageOnce(){
    ...
    function findComposer() {
      ...
      if (ae && (ae.isContentEditable || ae.tagName === 'TEXTAREA' || ae.tagName === 'INPUT')) return ae;
      return (
    ...
    function fallbackLink(el, url) {
      try {
        ...
        if (el.isContentEditable) {
          ...
          if (rg) { rg.deleteContents(); rg.insertNode(n); rg.setStartAfter(n); rg.setEndAfter(n); sel.removeAllRanges(); sel.addRange(rg); }
    ...
    window.vgInsertImage = async function vgInsertImage(dataUrl, filename = `screenshot-${Date.now()}.png`) {
      try {
        ...
        if (!ok && target) {
          const el = target.isContentEditable ? target : (document.querySelector('[contenteditable="true"]') || target);
          ok = tryDrop(el, file);
        }
        if (!ok) fallbackLink(target || document.body, dataUrl);
        return true;
      } catch (e) {
...
try { window.__VG_DRAG_BEGIN_FROM_TOP__?.(pd0.X, pd0.Y); } catch {}

// Silence iframe clicks for the whole drag
try {
  document.getElementById('__vg_iframe_hud__')
    ?.contentWindow?.postMessage({ source:'VG', type:'DRAG_SILENCE' }, '*');
} catch {}
  ...
  try { window.__VG_DRAG_MOVE_FROM_TOP__?.(base.X + dx, base.Y + dy); } catch {}
  return;
}
...
if (m.type === 'DRAG_END' || m.type === 'DRAG_DISARM') {
  ...
  try {
    document.getElementById('__vg_iframe_hud__')
      ?.contentWindow?.postMessage({ source:'VG', type:'DRAG_UNSILENCE' }, '*');
  } catch {}
  try { window.__VG_DRAG_END_INERT__?.(); } catch {}
  ...
  if (started) {
    try {
      ...
      let dxPx = 0, dyPx = 0;
      if (corner === 'tl') { dxPx = fr.left;                  dyPx = fr.top; }
      if (corner === 'tr') { dxPx = fr.left + size - vw;      dyPx = fr.top; }
      if (corner === 'bl') { dxPx = fr.left;                  dyPx = fr.top + size - vh; }
      if (corner === 'br') { dxPx = fr.left + size - vw;      dyPx = fr.top + size - vh; }
      ...
      chrome.runtime.sendMessage(
        { type: 'VG_SAVE_PILL_POS', host, path, dx: Math.round(dxPx), dy: Math.round(dyPx), anchor_corner: corner },
        () => void chrome.runtime.lastError
      );

      // Update local placement immediately so next paint uses window-fixed
      const p = window.__VG_DB_PLACEMENT || {};
      window.__VG_DB_PLACEMENT = { ...p, dx: Math.round(dxPx), dy: Math.round(dyPx), anchor_corner: corner };
  ...
  try {
    ...
    if (s) { s.style.pointerEvents='none'; s.style.width='0px'; s.style.height='0px'; s.style.cursor='grab'; }
  } catch {}
  return;
}
  ...
  function __vgEnsureDragShield__() {
    try {
      ...
      shield.style.cssText = [
        ...
        'z-index:2147483601','pointer-events:none','background:transparent',
      ...
      let frameStartLeft = 0, frameStartTop = 0;
      ...
      const enableShield  = () => { syncShieldToFrame(); shield.style.pointerEvents='auto'; shield.style.cursor='grab'; };
      const disableShield = () => { shield.style.pointerEvents='none'; shield.style.width='0px'; shield.style.height='0px'; };

      const onKey = (e) => { if (e.key === 'Shift') (e.type === 'keydown' ? enableShield() : disableShield()); };
...
shield.addEventListener('pointerdown', (ev) => {
  try {
    ...
    try { vgStopSticky(); } catch {}
    // also stop the in-frame sticky cleanup if present
    try { frameEl()?.__VG_STICKY_CLEANUP__?.(); } catch {}
...
let __vgMoveRAF = 0, __vgMoveX = 0, __vgMoveY = 0;
window.__VG_DRAG_MOVE_FROM_TOP__ = function (X, Y) {
  try {
    if (!dragging) return false;
    __vgMoveX = X; __vgMoveY = Y;
    if (!__vgMoveRAF) {
...
window.addEventListener('pointermove', (ev) => {
  ...
  if (!dragging) {
    if (window.__VG_DRAG_ARMED__ && (ev.buttons & 1)) {
      ...
      try { frameEl()?.contentWindow?.postMessage({ source:'VG', type:'DRAG_CONFIRMED' }, '*'); } catch {}
    } else {
...
window.addEventListener('pointerup', (ev) => {
  ...
  try {
    ...
    let dxStore = 0, dyStore = 0;
    if (corner === 'tl') { dxStore = fr.left;                  dyStore = fr.top; }
    if (corner === 'tr') { dxStore = fr.left + size - vw;      dyStore = fr.top; }
    if (corner === 'bl') { dxStore = fr.left;                  dyStore = fr.top + size - vh; }
    if (corner === 'br') { dxStore = fr.left + size - vw;      dyStore = fr.top + size - vh; }

    const host = location.hostname.toLowerCase().replace(/^www\./,'');
    const path = location.pathname || '/';

    chrome.runtime.sendMessage(
      { type: 'VG_SAVE_PILL_POS', host, path, dx: Math.round(dxStore), dy: Math.round(dyStore), anchor_corner: corner },
      () => void chrome.runtime.lastError
    );

    const p = window.__VG_DB_PLACEMENT || {};
    window.__VG_DB_PLACEMENT = { ...p, dx: Math.round(dxStore), dy: Math.round(dyStore), anchor_corner: corner };

    // Bake visual transform (we still have dxPx/dyPx from native path)
    const dxPx = ev.clientX - startX;
    const dyPx = ev.clientY - startY;
    try { window.__VG_BAKE_TRANSFORM__?.(dxPx, dyPx); } catch {}
...
window.__VG_INIT_HUD__ = function initHUD(opts) {
  ...
  try {
    ...
    if (!frame) {
      ...
      frame.style.cssText = [
        "position:fixed",
        "left:auto","right:auto","top:auto","bottom:auto", // neutral
    ...
    frame.addEventListener('load', () => {
      ...
      const iconIdle   = icons.iconIdle   || chrome.runtime.getURL('assets/inactive-pill.svg');
      const iconActive = icons.iconActive || chrome.runtime.getURL('assets/active-pill.svg');
      const p          = window.__VG_DB_PLACEMENT || {};
...
try {
  ...
  const iconIdle   = stored.iconIdle   || chrome.runtime.getURL('assets/inactive-pill.svg');
  const iconActive = stored.iconActive || chrome.runtime.getURL('assets/active-pill.svg');
...
function vgStartSticky(pObj, anchorEl) {
  ...
  if (pObj && typeof pObj.anchor_corner === 'string' && pObj.anchor_corner) {
    vgStopSticky();
    return;
  }
  ...
  const schedule = () => {
    ...
    rafId = anchorWin.requestAnimationFrame(() => {
      rafId = null;
      if (typeof window.__VG_PLACE_HUD__IMPL__ === "function" && frame.__VG_LAST_P__) {
  ...
  if (anchorWin.ResizeObserver) {
    ...
    try { roDoc.observe(anchorDoc.documentElement); } catch {}
    try { roDoc.observe(anchorDoc.body); } catch {}
  }
  ...
  try {
    const container =
      (anchorEl && anchorEl.closest("form,[data-testid*='composer'],[class*='composer'],[class*='footer'],[class*='toolbar'],main,[role='main'],[class*='Layout'],[class*='Shell'],[class*='Container']"))
      || anchorDoc.body;
  ...
  try {
    const outerFrameEl = anchorDoc.defaultView && anchorDoc.defaultView.frameElement;
  ...
  let onAnyInput = null, rafUntil = 0, rafHandle = null;
  const tick = () => {
    ...
    try {
      ...
      if (a && a.getBoundingClientRect) {
        ...
        frame.__VG_LAST_ANCHOR_RECT__ = { left: r.left, top: r.top, w: r.width, h: r.height };
      }
    } catch {}
  ...
  if (isCursor) {
    ...
    const targets = [anchorEl, anchorEl?.parentElement, anchorDoc].filter(Boolean);
    const types = ["input","keyup","keydown","paste","cut","compositionend","wheel","pointerup"];
    try {
      targets.forEach(t => types.forEach(type => t.addEventListener(type, onAnyInput, true)));
    } catch {}
  }
  ...
  frame.__VG_STICKY_CLEANUP__ = () => {
    try {
      ...
      if (isCursor && onAnyInput) {
        const targets = [anchorEl, anchorEl?.parentElement, anchorDoc].filter(Boolean);
        const types = ["input","keyup","keydown","paste","cut","compositionend","wheel","pointerup"];
        targets.forEach(t => types.forEach(type => t.removeEventListener(type, onAnyInput, true)));
...
function vgStopSticky() {
  try { frame.__VG_STICKY_CLEANUP__ && frame.__VG_STICKY_CLEANUP__(); } catch {}
  ...
  frame.__VG_ANCHOR__ = null;
    ...
    function findComposerScoped(doc, selector, slot) {
      let nodes = [];
      if (selector) { try { nodes = Array.from(doc.querySelectorAll(selector)); } catch {}
      }
      if (!nodes.length) {
      ...
      const idx = (String(slot || "primary").toLowerCase() === "secondary" && vis.length > 1) ? 1 : 0;
    ...
    function findSendButtonScoped(doc, composer, sendSelector) {
      const container =
        composer.closest("form,[data-testid*='composer'],[class*='composer'],[class*='footer'],[class*='toolbar']") ||
        composer.parentElement || doc;

      if (sendSelector) {
        try {
          const cands = Array.from(container.querySelectorAll(sendSelector)).filter(visible);
          if (cands.length) return cands.sort((a,b)=>b.getBoundingClientRect().right - a.getBoundingClientRect().right)[0];
        } catch {}
      }
      const fallback = Array.from(container.querySelectorAll(
        "button[aria-label='Send'],[data-testid='send-button'],button[type='submit'],button:has(svg[aria-label='Send'])"
      )).filter(visible);
      if (fallback.length) return fallback.sort((a,b)=>b.getBoundingClientRect().right - a.getBoundingClientRect().right)[0];
      return null;
    }
...
function findMicButtonScoped(doc, composer, micSelector) {
  ...
  if (micSelector) {
    try {
      const cands = Array.from(container.querySelectorAll(micSelector)).filter(visible);
      if (cands.length) return cands.sort((a,b)=>b.getBoundingClientRect().right - a.getBoundingClientRect().right)[0];
    } catch {}
  }
  const fallback = Array.from(container.querySelectorAll(
...
function findPlusButtonScoped(doc, composer, plusSelector) {
  ...
  if (plusSelector) {
    try {
      const cands = Array.from(container.querySelectorAll(plusSelector)).filter(visible);
      if (cands.length) {
        return cands.sort((a,b)=>b.getBoundingClientRect().right - a.getBoundingClientRect().right)[0];
      }
    } catch {}
  }
  ...
  if (fallback.length) {
    return fallback.sort((a,b)=>b.getBoundingClientRect().right - a.getBoundingClientRect().right)[0];
  }
  ...
  try {
    ...
    const bandTop = compR.top - 10, bandBot = compR.bottom + 10;

    const cands = Array.from(container.querySelectorAll("button,[role='button']"))
      .filter(loVisible)
...
function getIframeOffset(frameEl) {
  ...
  try {
    ...
    while (cur) {
      ...
      cur = (win && win.frameElement && win.frameElement !== cur) ? win.frameElement : null;
    }
    return { ox, oy };
...
function __VG_PLACE_WINDOW_FIXED__(placement = {}) {
  ...
  if (!frame.isConnected) (document.documentElement || document.body).appendChild(frame);
  ...
  try {
    ...
    frame.contentWindow?.postMessage({
      ...
      size: pillSize,
      pillSize,
      iconIdle:   icons.iconIdle   || chrome.runtime.getURL('assets/inactive-pill.svg'),
      iconActive: icons.iconActive || chrome.runtime.getURL('assets/active-pill.svg')
    }, '*');
  } catch {}
  ...
  switch (corner) {
    case 'tl': left='0px';  top='0px';    break;
    case 'tr': right='0px'; top='0px';    break;
    case 'bl': left='0px';  bottom='0px'; break;
    case 'br':
    default:   right='0px'; bottom='0px'; break;
  }
  ...
  frame.style.transform = `translate(${Math.round(dx)}px, ${Math.round(dy)}px)`; // ← apply nudges
  return true;
}


  try { window.__VG_PLACE_WINDOW_FIXED__ = __VG_PLACE_WINDOW_FIXED__; } catch {}
...
function findChipsRow(doc) {
  const LABELS = [/^web app$/i, /^data app$/i, /^game$/i, /^web app\s*\(python\)$/i];
  const text = (el) => (el.textContent || el.getAttribute('aria-label') || '').trim();
  const btns = Array.from(doc.querySelectorAll('button,[role="button"]'))
    .filter(b => LABELS.some(r => r.test(text(b))));
  if (btns.length < 2) return null;

  // lowest common ancestor of those buttons
  const path = (n) => { const a=[]; for (let p=n; p; p=p.parentElement) a.push(p); return a; };
...
function placeAnchorRowBR(doc, frameElForOffset) {
  ...
  try { if (frame.parentElement !== document.documentElement) document.documentElement.appendChild(frame); } catch {}
...
function placeAnchorExact(doc, frameElForOffset, pObj) {
  const pLocal = pObj || {};
  const sel = String(pLocal.send_selector || pLocal.composer_selector || '').trim();
  if (!sel) return false;
  ...
  if (sel.startsWith('text=')) {
    ...
    try {
      const pool = Array.from(doc.querySelectorAll('button,[role="button"],a'));
      el = pool.find(node => {
...
try { document.getElementById('__vb_anchor_style__')?.remove(); } catch {}
try { frame.__VB_ANCHOR_CLEANUP__?.(); } catch {}
...
const tryPlaceOnce = () => {
  ...
  if (isReplitHome()) {
    const row = findChipsRow(doc);
    if (!row) return false;
    const localFrame = (row?.ownerDocument?.defaultView?.frameElement) || frameEl || null;
  ...
  if (strat === "center-below") {
    ...
    const host = String(location.hostname || '').toLowerCase();
    const isCursor = /(^|\.)cursor\.com$|(^|\.)cursor\.so$/.test(host);
    const monacoEditor = comp.matches?.('.monaco-editor textarea.inputarea') ? comp.closest('.monaco-editor') : null;
    const stableWrap =
      comp.closest("form,[data-testid*='composer'],[class*='composer'],[class*='footer'],[class*='toolbar'],div.relative,[class*='Prompt'],[class*='Editor'],[class*='Input']")
      || comp.parentElement || comp;
    const anchorForSticky = isCursor ? (monacoEditor || stableWrap) : comp;
    const localFrame = (anchorForSticky?.ownerDocument?.defaultView?.frameElement) || frameEl || null;
...
if (strat === "anchor-exact") {
  ...
  if (strat === "send-button") {
    let send = null;
    try { send = p.send_selector ? doc.querySelector(p.send_selector) : null; } catch (_) {}
    let comp = null;
    if (!send) {
    ...
    const localFrame = (send?.ownerDocument?.defaultView?.frameElement) || frameEl || null;
  ...
  if (strat === "mic-button") {
    let target = null;
    try { target = p.send_selector ? doc.querySelector(p.send_selector) : null; } catch (_) {}
    let comp = null;
    if (!target) {
      comp = findComposerScoped(doc, p.composer_selector, p.slot);
      if (comp) {
        target = findMicButtonScoped(doc, comp, p.send_selector) ||
                 findSendButtonScoped(doc, comp, p.send_selector);
      }
    }
    if (!target) return false;

    const localFrame = (target?.ownerDocument?.defaultView?.frameElement) || frameEl || null;
  ...
  if (strat === "plus-button") {
    ...
    let plus = null;
    try { plus = p.send_selector ? doc.querySelector(p.send_selector) : null; } catch (_) {}
    ...
    const localFrame = (plus?.ownerDocument?.defaultView?.frameElement) || frameEl || null;
...
if (opts.singleShot === true) return true;
  ...
  const recheck = () => {
    ...
    if (!frame.isConnected) (document.body || document.documentElement).appendChild(frame);
  ...
  try {
    ...
    const mo = new MutationObserver(() => { if (tryPlaceOnce()) mo.disconnect(); });
...
window.__VG_PLACE_HUD__ = async function placeHudOnce(placement, opts = {}) {
  ...
  try { window.__VG_DB_PLACEMENT = placement; window.__VG_LAST_PLACEMENT = placement; } catch {}
  ...
  const mode = String(placement?.pick_strategy || placement?.strategy || '').toLowerCase();
  if (mode === 'off' || mode === 'disabled') {
    ...
    while (Date.now() < deadline) {
      const ok = (window.__VG_PLACE_HUD__IMPL__?.(placement, { singleShot: true }) === true);
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  const currentSz =
    ...
    Math.round(Number((window.__VG_DB_PLACEMENT && window.__VG_DB_PLACEMENT.pill_size) || 36));
