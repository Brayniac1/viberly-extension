...
const { APP, Z, MENU_DX } = (window.__VG_CONSTS || { APP: "vibeguardian", Z: 2147483600, MENU_DX: 0 });

/* ===============================
   Phase 0 — Feature flag + messages
   =============================== */
// Global feature flags (default OFF). Dev can override with chrome.storage.local.
try { window.__VG_FEATURES = window.__VG_FEATURES || {}; } catch {}
try {
  ...
  chrome?.storage?.local?.get?.(['vg_feat_screenshot'], (o) => {
    if (typeof o?.vg_feat_screenshot === 'boolean') {
      try { window.__VG_FEATURES.screenshot_enabled = o.vg_feat_screenshot; } catch {}
    }
  });
} catch { /* no-op */ }
...
function __qmLog(...args){ try{ __VG_QM_DEBUG && console.debug('[VG][QM]', ...args);}catch{} }
...
function makeIconBtn({ id, title, html, onClick, variant }) {
  ...
  const svg = b.querySelector('svg'); if (svg) svg.style.display = 'block';

  // 🔵 NEW: purple hover for preview variant
  if (variant === 'preview') {
    b.addEventListener('mouseenter', () => { b.style.color = '#8B5CF6'; });
    b.addEventListener('mouseleave', () => { b.style.color = '#cfd4ff'; });
  }
...
if (!window.__VG_QM_WIRED__) {
  ...
  chrome.runtime?.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg?.type === 'VG_PING_QM') { try { sendResponse({ qm: true }); } catch {} return true; }
  });
...
chrome.runtime?.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'VG_PAYWALL_SHOW') {
    ...
    (async () => {
      try {
        const mod = await import(chrome.runtime.getURL('src/ui/paywall.js'));
  ...
  if (openEl) {
    try {
      if (typeof window.__VG_QM_CLOSE === 'function') window.__VG_QM_CLOSE();
      else { openEl.remove(); window.__VG_LAST_MENU_CLOSE = performance.now(); }
    } catch {}
    return;
  }
  ...
  if (r) {
    try { openQuickMenu(r); } catch (e) { console.warn('[VG/QM] open failed', e); }
  } else {
    setTimeout(() => {
      const r2 = __vgPillRect();
      if (r2) {
        try { openQuickMenu(r2); } catch (e) { console.warn('[VG/QM] open failed (retry)', e); }
      } else {
        const size = 36;
        const synth = {
        ...
        console.debug('[VG/QM] no HUD iframe; opening with synthetic rect', synth);
        try { openQuickMenu(synth); } catch (e) { console.warn('[VG/QM] open failed (synth)', e); }
      }
    }, 150);
  }
});
  ...
  window.addEventListener('message', (ev) => {
    ...
    if (m && m.source === 'VG' && m.type === 'VG_QM_TOGGLE') {
      const r = __vgPillRect();
      if (r) { try { openQuickMenu(r); } catch (e) { console.warn('[VG/QM] open failed (bridge)', e); } }
    }
  });
}
/* ==== end OPEN LISTENER ==== */
...
function __toLocalGuard(row) {
  ...
  const tags   = Array.isArray(row.tags) ? row.tags : (Array.isArray(row.labels) ? row.labels : []);
...
function __vgTryInsertViaDBSelector(text) {
  const sel = (window.__VG_DB_PLACEMENT && window.__VG_DB_PLACEMENT.composer_selector) || "";
  if (!sel) return false;
  ...
  if ('value' in el) {
    ...
    try { el.dispatchEvent(new InputEvent('input',  { bubbles:true, cancelable:true, composed:true })); } catch {}
    try { el.dispatchEvent(new Event('change',      { bubbles:true })); } catch {}
    queueMicrotask(() => { try { el.focus(); } catch {} });
    return true;
  }


  // Path B: contentEditable / ProseMirror
  const ce = el.isContentEditable ? el : (el.closest && el.closest('[contenteditable="true"]'));
  if (ce) {
...
(function(){
  ...
  const IS_CURSOR = /(^|\.)cursor\.com$|(^|\.)cursor\.so$/.test(location.hostname);
  ...
  function __vgSetValue(el, value) {
    try {
      ...
      try { el.value = value; el.dispatchEvent(new Event("input", { bubbles:true })); } catch {}
    }
  }
...
if (IS_BOLT) {
  ...
  window.vgInsertPrompt = function (text) {
    ...
    if (typeof __vgTryInsertViaDBSelector === 'function' && __vgTryInsertViaDBSelector(text)) return true;

    try {
      if (window.__VG_COMPOSER_INSERT && window.__VG_COMPOSER_INSERT(String(text || ""))) return true;
    } catch {}
    const target = vgFindComposerRoot();
    return vgInsertTextSmart(target.ownerDocument || document, String(text || ""));
  };
  ...
  window.setComposerGuardAndCaret = function (text /*, marker */) {
    ...
    return vgInsertTextSmart(targetRoot.ownerDocument || document, String(text || ""));
  };
    ...
    function __cursorComposer() {
      return document.querySelector('main [contenteditable="true"]') ||
    ...
    window.vgInsertPrompt = function (text) {
      ...
      if (!el) { alert("Couldn't find the chat input."); return false; }
      const t = String(text || "");
      try { el.focus(); } catch {}
    ...
    window.setComposerGuardAndCaret = function (text /*, marker */) {
      ...
      // Fallback: range insertion
      try {
        const r = (doc.getSelection && doc.getSelection()?.getRangeAt(0)) || null;
        const node = doc.createTextNode(payload);
        if (r) r.insertNode(node);
        const after = doc.createRange();
        after.setStartAfter(node); after.collapse(true);
        const s2 = doc.getSelection(); s2.removeAllRanges(); s2.addRange(after);
        el.dispatchEvent(new Event('input', { bubbles:true }));
        return true;
      } catch {}
      return false;
    ...
    function __isProseMirror(el) {
      if (!el) return false;
      const hasPM = (n) => !!(n && n.classList && n.classList.contains('ProseMirror'));
      return hasPM(el) || hasPM(el.parentElement) || !!el.closest?.('.ProseMirror');
    }
    ...
    function __pmInsert(el, text) {
      if (!el) { __qmLog('pmInsert: no element'); return false; }
      const doc = el.ownerDocument || document;
      __qmLog('pmInsert: begin', { id: el.id, cls: el.className, ce: el.isContentEditable });

      // Focus + move caret to end
      try { el.focus(); } catch (e) { __qmLog('pmInsert: focus error', e); }
      try {
        const sel = doc.getSelection && doc.getSelection();
        if (sel) {
          const r = doc.createRange();
      ...
      } catch (e) { __qmLog('pmInsert: caret error', e); }
      ...
      try {
        if (document.execCommand && document.execCommand('insertText', false, payload)) {
          el.dispatchEvent(new Event('input', { bubbles:true }));
          __qmLog('pmInsert: execCommand success');
          return true;
        }
      } catch (e) { __qmLog('pmInsert: execCommand error', e); }
      ...
      } catch (e) { __qmLog('pmInsert: beforeinput error', e); }
      ...
      } catch (e) { __qmLog('pmInsert: range error', e); }
      ...
      } catch (e) { __qmLog('pmInsert: synthetic paste error', e); }
      ...
      try {
        if (navigator.clipboard?.writeText) {
          return navigator.clipboard.writeText(payload).then(() => {
            ...
            } catch (e) { __qmLog('pmInsert: clipboard paste exec error', e); }
            __qmLog('pmInsert: clipboard+paste unsupported');
            return false;
          }).catch((e) => { __qmLog('pmInsert: clipboard writeText error', e); return false; });
        }
      } catch (e) { __qmLog('pmInsert: clipboard block', e); }
...
window.vgInsertPrompt = window.vgInsertPrompt || function(text) {
  ...
  // Prefer activeElement if it's a contentEditable; else fall back to known selectors
      ...
      const activeCE = (document.activeElement && document.activeElement.isContentEditable) ? document.activeElement : null;
      const el =
        activeCE ||
        document.querySelector('div#prompt-textarea.ProseMirror') ||
        document.querySelector('textarea, input[type="text"], [contenteditable="true"], [role="textbox"][contenteditable="true"]');

      if (!el) { alert("Couldn't find the chat input."); __qmLog('insert: no target'); return false; }

      const t = String(text || "");
      __qmLog('insert: target', { tag: el.tagName, id: el.id, cls: el.className, ce: el.isContentEditable });
      ...
      if (!target) { __qmLog('insert: no CE container'); return false; }
      ...
      if (__isProseMirror(target)) {
        ...
        return new Promise((res) => requestAnimationFrame(() => res(__pmInsert(target, t))))
          .then((r) => { __qmLog('insert: PM retry', r); return r; });
      }
      ...
      try {
        ...
        afterNode.setStartAfter(node); afterNode.collapse(true);
        sel.removeAllRanges(); sel.addRange(afterNode);
...
window.setComposerGuardAndCaret = window.setComposerGuardAndCaret || function(text /*, marker */) {
  const t = String(text || "");
  ...
  if (!el) {
    const send = document.querySelector('button:has(svg[aria-label="Send message"]), button[data-testid="send-button"]');
    const candidates = [
    ...
    if (candidates.length) {
      if (send) {
        ...
        candidates.sort((a,b)=> Math.abs(a.getBoundingClientRect().y - sy) - Math.abs(b.getBoundingClientRect().y - sy));
      }
      el = candidates[0];
    }
  }
  ...
  const ok = (function run() {
    ...
    if ((target.classList && target.classList.contains('ProseMirror')) || !!target.closest?.('.ProseMirror')) {
...
function vgFindComposerRoot() {
  const fromDB = (window.__VG_DB_PLACEMENT && window.__VG_DB_PLACEMENT.composer_selector) || "";
  ...
  const sel = (fromDB && String(fromDB).trim()) ? (fromDB + ',' + DEFAULTS) : DEFAULTS;
...
function vgInsertTextSmart(rootEl, text) {
  ...
  if (isCursor) {
    const monacoTA = root.querySelector?.('.monaco-editor textarea.inputarea');
    if (monacoTA) {
      try { monacoTA.focus(); } catch {}
      ...
      try {
        if (document.execCommand && document.execCommand('insertText', false, String(text || ""))) return true;
      } catch (_) {}
      ...
      try {
        ...
        monacoTA.focus();
        if (document.execCommand && document.execCommand('paste')) return true;
      } catch (_) {}
  ...
  const ce = root.querySelector?.('[contenteditable="true"], [role="textbox"][contenteditable="true"]');
  if (ce) {
    try {
  ...
  if (ti) {
    try {
      try { ti.focus(); } catch {}
      const start = Number.isFinite(ti.selectionStart) ? ti.selectionStart : (ti.value || "").length;
      ...
      const proto = ti.tagName === 'TEXTAREA' ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype;
      const desc  = Object.getOwnPropertyDescriptor(proto, 'value');
      ...
      const caret = (before + insert).length;
      try { ti.setSelectionRange(caret, caret); } catch {}
      return true;
    } catch (_) { /* fall through */ }
  }

  try { return document.execCommand('insertText', false, String(text || "")); } catch (_) {}
  return false;
}
...
async function sendBG(type, payload, timeoutMs = 1500) {
  ...
  function ask() {
    return new Promise((res) => {
      let done = false;
      const to = setTimeout(() => { if (!done) res('__TIMEOUT__'); }, timeoutMs);
      try {
...
async function __vgEnsureBGSessionFromSoT() {
  try {
    const sot = await new Promise(res =>
      chrome.storage.local.get('VG_SESSION', o => res(o?.VG_SESSION || null))
    );
    if (!sot || !sot.access_token || !sot.refresh_token || !Number.isFinite(sot.expires_at)) {
...
async function qaGetLocal() {
  return new Promise(res => chrome.storage.local.get([VG_QA_KEY], o => res(o[VG_QA_KEY] || [])));
}
async function qaSetLocal(ids) {
  return new Promise(res => chrome.storage.local.set({ [VG_QA_KEY]: ids }, res));
}


window.vgQAGet = window.vgQAGet || (async () => {
  try {
    const r = await sendBG('VG_LIST_QA_FAVORITES');
    if (r?.ok && Array.isArray(r.data)) {
      const ids = r.data.map(row => row.prompt_id);
      try { await qaSetLocal(ids); } catch {}
      return ids;
    }
  } catch (e) {
    console.warn('[VG][qm] vgQAGet bridge failed → falling back to local', e);
  }
  return qaGetLocal();
});


window.vgQAToggle = window.vgQAToggle || (async (id) => {
  try {
    const cur = await window.vgQAGet();
    const isFav = cur.includes(id);
const r = await sendBG(isFav ? 'VG_DELETE_QA_FAVORITE' : 'VG_UPSERT_QA_FAVORITE', { prompt_id: id });

    if (r?.ok) {
      const next = isFav ? cur.filter(x => x !== id) : [...cur, id];
      try { await qaSetLocal(next); } catch {}
      try { document.dispatchEvent(new CustomEvent('vg-qa-updated', { detail:{ ids: next } })); } catch {}
      return !isFav;
    }
  } catch (_) {}
  // local fallback
...
function __vgGetCache() {
  return new Promise(res => chrome.storage.local.get([VG_LIB_CACHE_KEY], o => res(o[VG_LIB_CACHE_KEY] || null)));
}
function __vgSetCache(payload) {
  return new Promise(res => chrome.storage.local.set({ [VG_LIB_CACHE_KEY]: payload }, () => res()));
}
async function __vgFetchPromptsViaBackground() {
  try {
    const resp = await sendBG('FETCH_PROMPTS');
    if (!resp || resp.ok !== true) return { ok:false, error: resp?.error || 'Fetch failed' };
...
async function __vgLoadPromptLibrary(force = false) {
  ...
  const fresh = cached && !force && (Date.now() - (cached.cached_at || 0) < VG_LIB_TTL_MS);
...
(async () => {
  ...
  try {
    ...
    try { document.dispatchEvent(new CustomEvent("vg-lib-ready")); } catch {}
  } catch {
...
try { window.__VG_QUICK_TEXTS = QUICK_TEXTS; } catch {}
...
async function __vgStorageArea() {
  try { await chrome.storage.sync.get(null); return chrome.storage.sync; }
  catch { return chrome.storage.local; }
}
...
Object.assign(hdrActions.style, {
  ...
  pointerEvents: 'auto' // header itself has pointerEvents:none, so make this clickable
	...
	function mkCircleBtn({ id, title, svg }) {
	  ...
	  const on  = () => { btn.style.background = '#242733'; btn.style.borderColor = '#3a3a46'; btn.style.color = '#7c3aed'; };
	  const off = () => { btn.style.background = '#1f1f26'; btn.style.borderColor = '#2a2a33'; btn.style.color = '#a1a1aa'; };
	...
	chatBtn.addEventListener('click', async (e) => {
	  ...
	  try {
	    if (typeof window.openAiChatModal !== 'function') {
	      await import(chrome.runtime.getURL('src/ui/ai-chat.js'));
	    }
	    try { closeMenu(); } catch {}
  ...
  shotBtn.addEventListener('click', async (e) => {
    ...
    try {
      if (typeof window.openScreenshotOverlay !== 'function') {
        await import(chrome.runtime.getURL('src/ui/screenshot.js'));
      }
...
const gearBtn = makeIconBtn({
  ...
  onClick: async (e) => {
    ...
    if (typeof window.openModal === 'function') { window.openModal('advanced'); return; }
    if (typeof window.__SB_OPEN_MODAL === 'function') { window.__SB_OPEN_MODAL('advanced'); return; }

    try {
      await import(chrome.runtime.getURL('src/ui/settings.js'));
      const fn = window.openModal || window.__SB_OPEN_MODAL;
      if (typeof fn === 'function') { fn('advanced'); return; }
...
hdrActions.appendChild(gearBtn);
	...
	(function addHeaderTooltips(){
	  ...
	  function positionFor(target){
	    ...
	    tip.style.left = Math.max(6, Math.min(left, wrap.clientWidth - tip.offsetWidth - 6)) + 'px';
...
function attach(target, label){
  ...
  target.addEventListener('mouseenter', () => {
    tip.textContent = label;
	    ...
	    target.addEventListener('focus',  () => { tip.textContent = label; tip.style.display = 'block'; positionFor(target); tip.style.opacity='1'; tip.style.transform='translateY(0)'; });
	    target.addEventListener('blur',   () => { tip.style.opacity='0'; tip.style.transform='translateY(-4px)'; setTimeout(()=>{tip.style.display='none';},120); });
...
function applyFilter(q) {
  const n = norm(q);
  let visible = 0, visCustom = 0, visTeam = 0, visStd = 0, visQuick = 0;  // ← NEW visTeam
  ...
  if (sections.custom.header)   sections.custom.header.style.display   = visCustom ? "" : "none";
  if (sections.custom.sep)      sections.custom.sep.style.display      = visCustom ? "" : "none";
  if (sections.team?.header)    sections.team.header.style.display     = visTeam   ? "" : "none";  // ← NEW
  if (sections.team?.sep)       sections.team.sep.style.display        = visTeam   ? "" : "none";  // ← NEW
  if (sections.standard.header) sections.standard.header.style.display = visStd ? "" : "none";
  if (sections.quick.header)    sections.quick.header.style.display    = visQuick ? "" : "none";
  if (sections.quick.sepAbove)  sections.quick.sepAbove.style.display  = visQuick ? "" : "none";
  if (sections.quick.sepBelow)  sections.quick.sepBelow.style.display  = visQuick ? "" : "none";
}
...
function openPromptPreview(opts) {
  ...
  header.style.cssText = [
    ...
    'padding:14px 16px','border-bottom:1px solid #23252E','font-weight:600'
  ].join(';');
  const hLeft = document.createElement('div'); hLeft.textContent = title;
  const hBtn  = makeIconBtn({ title:'Close', html:'<svg viewBox="0 0 24 24" width="18" height="18"><path d="M6 6l12 12M18 6L6 18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>', onClick: closePromptPreview });
  header.appendChild(hLeft); header.appendChild(hBtn);

  // Body (auto-height up to 70vh → avoids scrolling when possible)
  const bodyEl = document.createElement('div');
  bodyEl.style.cssText = 'padding:16px;white-space:pre-wrap;line-height:1.5;max-height:70vh;overflow:auto;';
...
footer.style.cssText = 'display:flex;gap:8px;justify-content:flex-start;padding:12px 16px;border-top:1px solid #23252E;';

// Close (left)
const btnClose  = document.createElement('button');
btnClose.className = 'vgqm-btn';
btnClose.textContent = 'Close';
btnClose.style.cssText = 'padding:8px 12px;border-radius:10px;border:1px solid #2A2D36;background:#151821;color:#E6E7EB;cursor:pointer;';
...
btnEdit.style.cssText = 'padding:8px 12px;border-radius:10px;border:1px solid #2A2D36;background:#1f1f26;color:#E6E7EB;cursor:pointer;';
btnEdit.addEventListener('click', async () => {
  try {
    if (typeof opts?.onEdit === 'function') {
      ...
      routeToEditPrompt(String(opts?.id || ''), String(opts?.title || ''), String(opts?.body || ''));
    }
  } catch {}
  closePromptPreview();
});
...
btnInsert.style.cssText = 'padding:8px 12px;border-radius:10px;border:1px solid #8B5CF6;background:#8B5CF6;color:#0B0C10;font-weight:600;cursor:pointer;';
btnInsert.addEventListener('click', async () => {
  try { await opts?.onInsert?.(); } catch {}
  closePromptPreview();
});
...
footer.appendChild(btnInsert);
  ...
  back.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { e.preventDefault(); closePromptPreview(); }
    if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) {
      e.preventDefault(); try { opts?.onInsert?.(); } catch {} closePromptPreview();
    }
  });
...
function routeToEditPrompt(id, name, body) {
  (async () => {
    ...
    if (typeof window.openModal !== 'function' && typeof window.__SB_OPEN_MODAL !== 'function') {
      try { await import(chrome.runtime.getURL('src/ui/settings.js')); } catch (_) {}
    }

    // Open Dashboard on "My Prompts"
    try { (window.openModal || window.__SB_OPEN_MODAL)?.('advanced'); } catch (_) {}

    // Wait for modal host, then ask it to open Edit with our data
    const HOST_ID = (window.__VG_CONSTS?.APP || 'vibeguardian') + '-modal-host';
    let tries = 0;
    const t = setInterval(() => {
      ...
      if (host && host.shadowRoot) {
        clearInterval(t);
        try {
          document.dispatchEvent(new CustomEvent('vg-edit-prompt', {
            detail: {
              id:   String(id   || ''),
              name: String(name || ''),
              body: String(body || '')
...
function __vgDecorateRowInteractivity(row) {
  ...
  if (reduceMotion) {
    ...
    const onNoScale = () => { fx.style.boxShadow = '0 0 0 1px rgba(124,58,237,.35) inset'; fx.style.borderColor='#7c3aed'; row.style.borderColor='#7c3aed'; };
    const offNoScale = () => { fx.style.boxShadow='none'; fx.style.borderColor='transparent'; row.style.borderColor='#22232b'; };
...
(() => {
  ...
  sepAboveQA.style.cssText = "margin:8px 0 6px;height:1px;background:#22232b";
  scroller.appendChild(sepAboveQA);
  sections.quick.sepAbove = sepAboveQA;

  const head = document.createElement("div");
  head.textContent = "My Quick Adds";
  head.style.cssText = "font:500 10px Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#cbd5e1;margin:2px 2px 8px";
  ...
  sepBelowQA.style.cssText = "margin:8px 0 6px;height:1px;background:#22232b";
  ...
  async function renderQuickAdds() {
    try {
      ...
      const lib = Array.isArray(window.__VG_PROMPT_LIBRARY) ? window.__VG_PROMPT_LIBRARY : [];
...
if (!lib.length) {
  ...
  return;
      ...
      favs.forEach(p => {
        const row = document.createElement("div");
        row.style.cssText = "position:relative;display:flex;align-items:center;gap:6px;padding:6px 4px;border-radius:8px;border:1px solid #22232b;margin-bottom:6px;background:#0c0e13;min-width:0;overflow:hidden;";
        __vgDecorateRowInteractivity(row);

        const label = document.createElement("div");
        label.textContent = p["Prompt Name"];
        label.style.cssText = "flex:1;min-width:0;font-size:14px;color:#e5e7eb;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;";
...
const btn = makeIconBtn({
  ...
  onClick: (e) => {
    ...
    openPromptPreview({
      ...
      onInsert: async () => { await insertQuickAdd(); },
      onEdit: async () => {
        routeToEditPrompt(String(p.id || ''), String(p["Prompt Name"] || ''), String(p["Prompt Text"] || ''));
      }
    });
  }
});
        ...
        async function insertQuickAdd() {
          ...
          try {
            const gate = await sendBG('VG_CAN_INSERT_QUICK', { prompt_id: String(p.id) });
            if (gate && gate.ok === false && (gate.reason === 'QUICK_ADD_LIMIT' || gate.reason === 'CUSTOM_GUARD_LIMIT')) {
              __qmLog('QA insert: BLOCKED by plan limit; paywall should pop');
              return;
            }
          } catch (e) { __qmLog('QA insert: pre-flight threw', e); return; }

          __qmLog('QA insert: start', { pid: String(p.id) });
          const text = p["Prompt Text"] || "";
          let ok = false;
          try { ok = !!window.vgInsertPrompt(text); } catch (e) { __qmLog('QA insert: vgInsertPrompt threw', e); ok = false; }
          if (!ok) {
            __qmLog('QA insert: first attempt failed → retry rAF');
            await new Promise(res => requestAnimationFrame(res));
            try { document.querySelector('div#prompt-textarea.ProseMirror')?.focus(); } catch {}
            try { ok = !!window.vgInsertPrompt(text); } catch (e) { __qmLog('QA insert: retry threw', e); ok = false; }
            __qmLog('QA insert: retry result', ok);
          }
          (async () => {
            try { const r = await sendBG('VG_LOG_QUICK_USE', { prompt_id: String(p.id) }); __qmLog('QA insert: usage log result', r && r.ok); } catch (e) { __qmLog('QA insert: usage log error', e); }
          })();
  ...
  const onLibReady = () => { try { renderQuickAdds(); } catch {} };
  const onLibUpdated = () => { try { renderQuickAdds(); } catch {} };
  const onQAUpdated = () => { try { renderQuickAdds(); } catch {} };
  ...
  try {
	...
	if (!Array.isArray(customs) || customs.length === 0) {
	  ...
	  async function openSettingsAdvanced(){
	    if (typeof window.openModal === "function") return void window.openModal("advanced");
	    if (typeof window.__SB_OPEN_MODAL === "function") return void window.__SB_OPEN_MODAL("advanced");
	    try {
	      await import(chrome.runtime.getURL("src/ui/settings.js"));
	      const fn = window.openModal || window.__SB_OPEN_MODAL;
	  ...
	  btn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); openSettingsAdvanced(); });
...
const btn = makeIconBtn({
  ...
  onClick: (e) => {
    ...
    openPromptPreview({
      ...
      body:  String(cg.body || ''),
      onInsert: async () => { await insertCustom(); },
      onEdit: async () => {
        routeToEditPrompt(String(cg.id || ''), String(cg.name || ''), String(cg.body || ''));
      }
    });
  }
});
...
async function insertCustom() {
  ...
  try {
    const gate = await sendBG('VG_CAN_INSERT_CUSTOM', { guard_id: String(cg.id) });
    if (gate && gate.ok === false && gate.reason === 'CUSTOM_GUARD_LIMIT') {
  ...
  let placed = false;
  try { placed = !!window.vgInsertPrompt(cg.body); } catch (e) { __qmLog('CG insert: vgInsertPrompt threw', e); placed = false; }

  // 1b) Fallback to the legacy caret path if needed
  if (!placed) {
    __qmLog('CG insert: vgInsertPrompt failed → try caret path');
    try { placed = !!setComposerGuardAndCaret(cg.body, ''); } catch (e) { __qmLog('CG insert: caret insert threw', e); placed = false; }
  }
  ...
  if (!placed) {
    ...
    await new Promise(res => requestAnimationFrame(res));
    try {
      const pm = document.querySelector('div#prompt-textarea.ProseMirror');
      if (pm) { try { pm.focus(); } catch {} }
    } catch {}
    try { placed = !!window.vgInsertPrompt(cg.body); } catch (e) { __qmLog('CG insert: retry threw', e); placed = false; }
  ...
  (async () => {
    try {
      const r = await sendBG('VG_LOG_GUARD_USE', { guard_id: String(cg.id) });
  ...
  try { await maybePromptUpgrade(); } catch {}
  document.getElementById('vg-quick-menu')?.remove();
}
...
const eye = makeIconBtn({
  ...
  onClick: (e) => {
    ...
    openPromptPreview({
      ...
      body:  String(tp.body || ''),
      onInsert: async () => { await insertTeam(tp); }
    });
  }
});
      ...
      async function insertTeam(item) {
        ...
        try {
          const gate = await sendBG('VG_CAN_INSERT_CUSTOM', { guard_id: String(item.id) });
          if (gate && gate.ok === false && gate.reason === 'CUSTOM_GUARD_LIMIT') {
            __qmLog('Team insert: BLOCKED by plan limit; paywall should pop');
            return;
          }
        } catch { return; }

        let placed = false;
        try { placed = !!window.vgInsertPrompt(item.body); } catch { placed = false; }
        if (!placed) {
        ...
        (async () => {
          try { await sendBG('VG_LOG_GUARD_USE', { guard_id: String(item.id) }); } catch {}
        })();
...
/* ---------- BUILT-IN GUARDS (BELOW) ---------- */
	async function renderStandardGuards() {
	  try { builtinsSection.innerHTML = ""; } catch {}
...
document.addEventListener("vg-standard-guards-updated", () => {
  try { renderStandardGuards(); } catch {}
});
...
requestAnimationFrame(() => {
  ...
  const pill = document.getElementById(APP + "-pill-host")
               ?.shadowRoot?.getElementById("vg-pill");
  const r = pill?.getBoundingClientRect?.() || { left: prLeft, top: prTop, width: prWidth, height: 0 };
...
const closeMenu = () => {
  ...
  try { document.removeEventListener('pointerdown', onDocPointerDown, true); } catch {}
  try { document.removeEventListener('click', onDocClick, true); } catch {}
  try { window.removeEventListener('keydown', onKeyDown, true); } catch {}
  try { __vgStopAnchorToPill(); } catch {}       // ← stop rAF + viewport listeners
  try { wrap.remove(); } catch {}
  // clear exported closer after tear-down
  try { delete window.__VG_QM_CLOSE; } catch {}
};

// export the real closer for toggle & other callers
try { window.__VG_QM_CLOSE = closeMenu; } catch {}
...
function onDocPointerDown(ev) {
  ...
  if (document.getElementById('vgqm-preview-backdrop')?.contains(ev.target)) return;
...
function __vgStartAnchorToPill(wrap) {
  if (!wrap) return;
  if (__vgQM_RAF) { cancelAnimationFrame(__vgQM_RAF); __vgQM_RAF = 0; }
  ...
  try { window.visualViewport?.addEventListener('resize', onVV, { passive: true }); } catch {}
  try { window.visualViewport?.addEventListener('scroll', onVV, { passive: true }); } catch {}
  wrap.__vg_vv_off = () => {
    try { window.visualViewport?.removeEventListener('resize', onVV, { passive: true }); } catch {}
    try { window.visualViewport?.removeEventListener('scroll', onVV, { passive: true }); } catch {}
  };
}
...
try {
  window.__VG_CLICK_TAP__ && document.removeEventListener('click', window.__VG_CLICK_TAP__, true);
