...
(() => {
  ...
  const ICON_OPEN_EXTERNAL =
    '<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 3h7v7"/><path d="M21 3l-11 11"/><path d="M10 3h-3a3 3 0 0 0-3 3v11a3 3 0 0 0 3 3h11a3 3 0 0 0 3-3v-3"/></svg>';
  ...
  async function openAiChatViaSlash() {
    try {
      if (typeof window.openAiChatModal !== "function") {
        const url = browser?.runtime?.getURL?.("src/ui/ai-chat.js");
        if (url) {
          await import(url);
        }
      }
  ...
  function captureCaretRange(anchor) {
    ...
    try {
      const sel =
        anchor.ownerDocument?.getSelection?.() || window.getSelection?.();
  ...
  function handleSelectionChange() {
    ...
    const anchor =
      state.anchorEl && document.contains(state.anchorEl)
        ? state.anchorEl
        : state.composer;
  ...
  function handleClickAway(ev) {
    ...
    if (
      state.composer &&
      (state.composer === target || state.composer.contains(target))
    )
      return;
    closePalette();
  }
  ...
  function handleKeyDown(ev) {
    ...
    if (key === "Enter") {
      ev.preventDefault();
      ev.stopPropagation();
      applySelection(state.selectedIndex).catch((e) =>
        console.warn("[VG][slash] insert failed", e)
      );
      return;
    }
    if (key === "Escape") {
    ...
    state.matches.forEach((match, index) => {
      ...
      if (match.icon === "external") {
        ...
        openBtn.addEventListener("click", (e) => {
          e.preventDefault();
          e.stopPropagation();
          applySelection(index).catch((err) =>
            console.warn("[VG][slash] command click failed", err)
          );
        });
        ensureRight().appendChild(openBtn);
      } else if (
        typeof window.__VG_OPEN_PROMPT_PREVIEW === "function" &&
        match.previewable !== false
      ) {
        const eyeBtn = document.createElement("button");
        eyeBtn.type = "button";
        eyeBtn.className = "vg-slash-icon vg-slash-eye";
        eyeBtn.innerHTML =
          '<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"></path><circle cx="12" cy="12" r="3"></circle></svg>';
      ...
      row.addEventListener("click", () => {
        applySelection(index).catch((err) =>
          console.warn("[VG][slash] insert click failed", err)
        );
      });
  ...
  async function performInsert(match, trigger) {
    ...
    if (inserted) {
      try {
        await maybeLogUsage(match);
      } catch (e) {
        console.warn("[VG][slash] usage log failed", e);
      }
      try {
  ...
  async function gateForPrompt(match) {
    try {
      if (match.source === "quick") {
        const guard = await sendBG("VG_CAN_INSERT_QUICK", {
          prompt_id: match.id,
        });
        if (guard && guard.ok === false) return false;
      } else if (match.source === "custom" || match.source === "team") {
        const guard = await sendBG("VG_CAN_INSERT_CUSTOM", {
          guard_id: match.id,
        });
        if (guard && guard.ok === false) return false;
      }
  ...
  function extractSlashTrigger(composer) {
    ...
    if ("value" in composer) {
      ...
      const pos = Number.isFinite(composer.selectionStart)
        ? composer.selectionStart
        : value.length;
      const snippet = stripZeroWidth(
        value.slice(Math.max(0, pos - LOOKBACK_CHARS), pos)
      );
    ...
    const query = match[2];
    const removeLen = query.length + 1;
    const caretInfo = findTextPositionBackward(
      composer,
      range.endContainer,
      range.endOffset,
      removeLen
  ...
  function positionPalette(composer, palette) {
    ...
    left = Math.max(
      padding,
      Math.min(left, window.innerWidth - rootRect.width - padding)
    );
    top = Math.max(
      padding,
      Math.min(top, window.innerHeight - rootRect.height - padding)
    );
  ...
  function getAnchorRect(composer) {
    ...
    if (
      trigger?.range &&
      typeof trigger.range.getBoundingClientRect === "function"
    ) {
  ...
  function ensurePromptLibrary() {
    if (Array.isArray(window.__VG_PROMPT_LIBRARY))
      return Promise.resolve(window.__VG_PROMPT_LIBRARY);
  ...
  function getComposerFromEvent(target) {
    ...
    if (target.closest) {
      return target.closest(
        'textarea, input[type="text"], input[type="search"], [contenteditable="true"], [role="textbox"][contenteditable="true"]'
      );
    }
    return null;
  }
  ...
  function registerScrollListeners(composer) {
    ...
    if (parent && parent !== document && parent !== window)
      targets.push(parent);
