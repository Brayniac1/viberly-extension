...
(() => {
	...
	(async () => {
	  async function seedBGFromPageSession() {
	    try {
	      const resp = await (window.VG?.auth?.getSession?.() ?? Promise.resolve({ data:{ session:null } }));
...
async function sendBG(type, payload, timeoutMs = 1500) {
  ...
  if (r === '__NO_RECEIVER__' || r === '__TIMEOUT__') {
    ...
    try {
      const resp = await (window.VG?.auth?.getSession?.() ?? { data:{ session:null } });
      const s = resp?.data?.session;
      if (s?.access_token && s?.refresh_token) {
...
function buildInjection(active, selectedCustomGuards = []) {
  ...
  for (const cg of (selectedCustomGuards || [])) if (cg?.body) b.push(cg.body.trim());
  ...
  return b.join("\n\n");
	...
	const vgGetCache = () => new Promise(res =>
	  chrome.storage.local.get([VG_LIB_CACHE_KEY], o => res(o[VG_LIB_CACHE_KEY] || null))
	);
	const vgSetCache = (payload) => new Promise(res =>
	  chrome.storage.local.set({ [VG_LIB_CACHE_KEY]: payload }, () => res())
	);
	...
	async function vgLoadLocalSeed() {
	  try {
	    if (!window.PROMPT_LIBRARY_SEED) {
	      const url = chrome.runtime.getURL("vg_library.js");
...
async function vgFetchGuardsViaBackground() {
  try {
    await __vgEnsureBGSessionFromSoT();
    const resp = await sendBG('FETCH_PROMPTS', {
      filter: { type: 'Programming', subcategory: 'Standard Guards', status: 'published' }
    });
    if (!resp || resp.ok !== true) return { ok:false, error: resp?.error || 'Fetch failed' };
    return { ok:true, items: resp.items || [] };
  } catch (e) {
...
async function vgLoadGuardTemplatesFromDB() {
  try {
    ...
    const mapIdToKey = {
      basic:'BASIC', ui:'UI', copy:'COPY', logic:'LOGIC',
      strict:'STRICT', noref:'NOREF', wire:'WIRE', pol:'POL', data:'DATA', qa:'QA'
    };
	...
	async function qaActiveSetMap(map) {
	  ...
	  try { document.dispatchEvent(new CustomEvent('vg-qa-active-updated')); } catch {}
	}
	async function qaActiveSet(id, on) {
	  const m = await qaActiveGetMap();
	  if (on) m[id] = true; else m[id] = false;
	...
	async function vgQAGet() {
	  try {
	    const r = await new Promise(res =>
	      chrome.runtime.sendMessage({ type: 'VG_LIST_QA_FAVORITES' }, res)
	    );
  ...
  try {
	...
	async function vgQAToggle(id) {
	  try {
	    ...
	    const r = await new Promise(res =>
	      chrome.runtime.sendMessage(
	        { type: isFav ? 'VG_DELETE_QA_FAVORITE' : 'VG_UPSERT_QA_FAVORITE', prompt_id: id },
	        res
	      )	
	    );
	    if (r?.ok) {
...
function vgRenderMarketplaceGrid(container) {
  const items = Array.isArray(window.__VG_PROMPT_LIBRARY) ? window.__VG_PROMPT_LIBRARY : [];
...
`;
  ...
  try {
    ...
    setCols(); mq.addEventListener ? mq.addEventListener('change', setCols) : mq.addListener(setCols);
  } catch {}


  // ── Helpers
  const getPromptUrl = (id) => `https://viberly.ai/prompt/${encodeURIComponent(id)}`;  // ← new public route
  const titleOf = (p) => String(p["Prompt Name"] || '').trim();
  const descOf  = (p) => {
    const s = String(p["Tagline"] || '').trim();
    if (s) return s;
    const fall = String(p["Prompt Text"] || '').replace(/\s+/g,' ').trim();
...
async function handleBuyClick(promptId) {
  try {
    ...
    const resp = await sendBG('VG_START_PROMPT_CHECKOUT', { prompt_id: String(promptId) }, 5000);

    if (!resp?.ok) {
      if (resp?.reason === 'NO_SESSION') {
        // Not signed in inside the extension → open the sign-in popup once
        try { await __vgPromptSignInPopup?.(); } catch {}
    ...
    try { chrome.tabs?.create ? chrome.tabs.create({ url }) : window.open(url, '_blank'); }
    catch { window.open(url, '_blank'); }

    // When the user returns, refresh purchases so the card flips to "Purchased"
    const onFocus = async () => {
      window.removeEventListener('focus', onFocus);
      try { await vgLoadOwnershipSets(); } catch {}
      try { typeof doPaint === 'function' && doPaint(true); } catch {}
    };
  ...
  const allCats = (() => {
    ...
    items.forEach(p => { if (p["Type"]) set.add(String(p["Type"])); });
    return [...set];
  })();
  ...
  function paintPills(){
    ...
    function applyPillStyles(){
      btns.forEach(({btn, cat}) => {
        ...
        btn.style.boxShadow   = isActive ? '0 0 0 1px rgba(124,58,237,.25) inset' : 'none';
      });
    }
  ...
  function filterByQuery(list, q){
    ...
    return list.filter(p =>
      ...
      String(p["Tagline"]||'').toLowerCase().includes(n) ||
      String(p["Type"]||'').toLowerCase().includes(n) ||
      String(p["Subcategory"]||'').toLowerCase().includes(n) ||
      (Array.isArray(p["Labels"]) ? p["Labels"] : []).some(t => String(t).toLowerCase().includes(n))
    );
  }
  ...
  function cardFor(p){
    ...
    `;
    card.onmouseenter = () => { card.style.borderColor = '#4c1d95'; card.style.boxShadow='0 12px 40px rgba(0,0,0,.45)'; };
    card.onmouseleave = () => { card.style.borderColor = '#1e2230'; card.style.boxShadow='none'; };
    ...
    if (avatarUrl) {
      ...
      img.style.cssText = 'width:36px;height:36px;border-radius:10px;display:block;object-fit:cover';
      av.appendChild(img);
      av.style.cssText = 'width:36px;height:36px;border-radius:10px;display:block';
    } else {
      av.style.cssText = 'width:36px;height:36px;border-radius:10px;background:#1f1f26;color:#fff;display:grid;place-items:center;font-weight:700';
      av.textContent = '👤';
    }
    ...
    name.style.cssText='font-weight:600;color:#e5e7eb;max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis';

    left.appendChild(av); left.appendChild(name);

    const pop = document.createElement('a');
    pop.href = getPromptUrl(p.id);
    pop.target = '_blank'; pop.rel='noreferrer';
    ...
    pop.addEventListener('click', (e) => {
      try {
        ...
        // last resort
        try { window.open(getPromptUrl(p.id), '_blank', 'noopener,noreferrer'); } catch {}
      }
    });

    top.appendChild(left); top.appendChild(pop);
    ...
    tags.style.cssText='display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:6px';
    tagsOf(p).forEach(t => {
      const chip = document.createElement('span');
      chip.textContent = t;
      chip.style.cssText='display:inline-block;margin-right:6px;font-size:11px;color:#cbd5e1;border:1px solid #2a2a33;border-radius:999px;padding:2px 8px;vertical-align:middle';
    ...
    foot.style.cssText='margin-top:10px;display:flex;align-items:center;justify-content:space-between;gap:8px';
    ...
    cta.onclick = async (e) => {
      ...
      if (paid) {
        ...
        try {
          ...
          if (nowAdded) __VG_FAV_SET.add(pid); else __VG_FAV_SET.delete(pid);
...
vgLoadOwnershipSets().then(() => doPaint()).catch(() => doPaint());


  let t = null;
  searchEl.addEventListener('input', () => {
    clearTimeout(t); t = setTimeout(doPaint, 120);
  });
}
...
(async () => {
  try {
	...
	const qbtn = (root = document) => Array.from(root.querySelectorAll('button,[role="button"]'));
	...
	window.__VG_TEST_FETCH = async () => {
	  const r = await new Promise((resolve) =>
	    chrome.runtime.sendMessage({ type: 'FETCH_PROMPTS' }, resolve)
	  );	
  ...
  function findRowEditChat() {
	...
	window.vgFindComposer = window.vgFindComposer || function () {
	  const sel = 'textarea,[role="textbox"],[contenteditable="true"],[contenteditable]';
	  const list = Array.from(document.querySelectorAll(sel)).filter(vgIsVisible);
  ...
  function setComposerGuardAndCaret(textBlock /*, marker */) {
	  ...
	  const cur = ("value" in ta) ? (ta.value || "") : (ta.innerText || "");
	  const next = String(textBlock || "").trimEnd() + (cur ? ("\n\n" + cur.trimStart()) : "");
  ...
  async function S_get(key, def = null) {
    ...
    return Object.prototype.hasOwnProperty.call(obj, "sb_" + key) ? obj["sb_" + key] : def;
  }
...
async function __vgPromptSignInPopup() {
  try {
    const r = await chrome.runtime.sendMessage({ type: 'VG_OPEN_SIGNIN_POPUP' });
...
async function saveSettingsRoamingOrPromptLogin(patch) {
  ...
  return !!ok;
	...
	try { window.maybePromptUpgrade = maybePromptUpgrade; } catch {}
    ...
    function __vgCategory() {
      ...
      try { return (window.VG && VG.utils.getCategoryForHost(location.hostname)) || 'general'; }
...
function __toLocalGuard(row) {
  return {
    ...
    body: row.body || '',
    tags: Array.isArray(row.tags) ? row.tags : (Array.isArray(row.labels) ? row.labels : []),
    status: row.status || 'inactive',
    createdAt: row.created_at ? Date.parse(row.created_at)
                              : (typeof row.updatedAt === 'number' ? row.updatedAt : Date.now()),
    updatedAt: row.updated_at ? Date.parse(row.updated_at)
                              : (typeof row.updatedAt === 'number' ? row.updatedAt : Date.now()),
...
async function CG_list(_scope = 'site') {
  ...
  return Array.isArray(items) ? items.map(__toLocalGuard) : [];
    ...
    async function CG_saveAll(items) { return items; }
...
async function CG_create({ name, body, tags = [] }) {
  ...
  if (!resp?.ok || !resp?.row) throw new Error(resp?.error || 'CREATE_FAILED');
...
async function CG_update(id, patch) {
  ...
  if (!resp?.ok || !resp?.row) throw new Error(resp?.error || 'UPDATE_FAILED');
  return __toLocalGuard(resp.row);
}
...
async function CG_delete(id) {
  ...
  return true;
    ...
    const PRESETS = [
	  { id: "all",    name: "Beginner Safe Mode (all)", on: ["basic", "ui", "copy", "logic", "strict", "noref", "wire", "pol", "data"] },
	  { id: "design", name: "Design Guard",             on: ["ui", "copy"] },
	  { id: "logic",  name: "Logic Guard",              on: ["logic", "noref", "wire", "data"] },
	  { id: "bugfix", name: "Bugfix Surgical Mode",     on: ["copy", "logic", "strict", "noref"] },
	  { id: "ship",   name: "Ship-Safe Feature Build",  on: ["logic", "strict", "noref", "wire", "pol", "data"] },
	];

	// --- NEW: helpers to infer preset or "custom" from a list of toggles
	function _norm(arr){ return [...new Set(arr || [])].sort(); }
  ...
  // ===== Modal (v0.4.9) =====
	async function openModal(defaultTab = 'advanced') {
	  ...
	  try {
	    ...
	    try {
	      const resp = await (window.VG?.auth?.getSession?.() ?? Promise.resolve({ data: {} }));
	      const s = resp?.data?.session;
	    ...
	    if (typeof signedFlag !== 'boolean') {
	      const auth = await new Promise((res) => {
	        let done = false;
	        const to = setTimeout(() => { if (!done) res(undefined); }, 400);
	        try {
	          chrome.runtime.sendMessage({ type: 'AUTH_STATUS' }, (r) => {
	            done = true; clearTimeout(to); res(r);
	          });
	        } catch { res(undefined); }
	      });
	    ...
	    if (typeof signedFlag !== 'boolean') signedFlag = true;
	        ...
	        if (signedFlag === false) {
		    await new Promise(res => chrome.runtime.sendMessage({ type: 'OPEN_POPUP' }, res));
		    return;
		  }
		} catch {}
	...
	const LOGO_URL = chrome.runtime.getURL('assets/Viberly-transparent.svg');
	shadow.innerHTML = `
	  <style>
...
function __vgSettingsPaint(data) {
  ...
  const hostFromPage = location.hostname.toLowerCase().replace(/^www\./,'');
  const host = data?.host || hostFromPage || null;

  const tri  = (typeof data?.state === 'string') ? data.state : (data?.enabled ? 'on' : 'off');
  const isOn = (tri === 'on');
  const isNA = (tri === 'na');
  ...
  if (isNA) toggle.setAttribute('aria-disabled','true'); else toggle.removeAttribute('aria-disabled');
...
async function __vgResetPill(scope /* 'page' | 'site' */) {
  ...
  try {
    // 1) Delete override(s)
    const r = await sendBG('VG_RESET_PILL_POS', { host, scope: (scope === 'site' ? 'site' : 'page'), path });
    if (!r || r.ok !== true) { alert('Reset failed. Please try again.'); return; }

    // 2) Pull fresh merged placement (global + any remaining override)
    const resp = await sendBG('VG_GET_PAGE_PLACEMENTS', { host, path });
    const rules = (resp && resp.ok && Array.isArray(resp.placements)) ? resp.placements : [];
    ...
    if (pick) {
      ...
      try { window.__VG_DB_PLACEMENT = next; } catch {}
...
function __vgSettingsWire() {
  ...
  if (toggle && !toggle.__wired) {
    ...
    toggle.addEventListener('change', async () => {
      if (toggle.disabled) { toggle.checked = false; return; }
      const wantOn = !!toggle.checked;
      const host = (q('#saHost')?.textContent || '').trim().toLowerCase().replace(/^www\./,'') || null;
      try { await sendBG('SET_SITE_ACCESS', { host, state: wantOn ? 'inherit' : 'off' }); } catch {}
      __vgSettingsRefresh();
    });
  }
  ...
  if (btnResetPage && !btnResetPage.__wired) {
    ...
    btnResetPage.addEventListener('click', async () => {
      ...
      try { await __vgResetPill('page'); }
      finally { btnResetPage.textContent = old; btnResetPage.disabled = false; }
    });
  }
  ...
  try {
    chrome.runtime.onMessage.addListener((msg) => {
      ...
      const shownHost = (q('#saHost')?.textContent || '').trim().toLowerCase().replace(/^www\./,'');
      const msgHost   = String(msg?.host || '').toLowerCase().replace(/^www\./,'');
      if (shownHost && msgHost && shownHost === msgHost) __vgSettingsRefresh();
    });
  } catch {}
}
...
async function __vgMaybeAddTeamTabShell() {
 try {
    const resp = await new Promise(res => chrome.runtime.sendMessage({ type: 'GET_TEAM_PROMPTS' }, res));
...
tabsBar.addEventListener('click', (ev) => {
  ...
  const sections = ['advanced', 'team', 'library', 'settings', 'billing'];

  // underline exactly one tab
  tabsBar.querySelectorAll('.tab')
    .forEach(t => t.classList.toggle('active', t === btn));

  // show exactly one section
  sections.forEach(k => {
    const s = document
      .getElementById((window.__VG_CONSTS?.APP||'vibeguardian') + '-modal-host')
      ?.shadowRoot?.querySelector(`section[data-tab="${k}"]`);
    if (s) s.hidden = (k !== which);
  });

  if (which === 'team') {
    try { __vgPaintTeamPromptsList?.(); } catch {}
  }
}, { capture: true });
    ...
    sec.querySelector('#tpRefresh')?.addEventListener('click', async () => {
      await new Promise(res => chrome.runtime.sendMessage({ type: 'REFRESH_TEAM_PROMPTS' }, res));
...
async function __vgPaintTeamPromptsList() {
  const host = (document.getElementById((window.__VG_CONSTS?.APP||'vibeguardian') + '-modal-host') || {}).shadowRoot;
  ...
  const resp = await new Promise(res => chrome.runtime.sendMessage({ type:'GET_TEAM_PROMPTS' }, res));
  const items = (resp && resp.ok && Array.isArray(resp.prompts)) ? resp.prompts : [];
  ...
  function render() {
    ...
    if (!filtered.length) {
      root.innerHTML = '<div class="muted">No team prompts match your search.</div>';
      return;
    }
    ...
    filtered.forEach(item => {
      ...
      tog.onclick = async (e) => {
        ...
        const r = await new Promise(res => chrome.runtime.sendMessage(
          { type:'VG_GUARD_SET_STATUS', id: item.id, on: nextOn }, res
        ));
        if (!r?.ok) {
          // rollback on error
          tog.classList.toggle('on', !nextOn);
      ...
      btnPrev.onclick = async (e) => {
        e.preventDefault(); e.stopPropagation();
        try { await navigator.clipboard.writeText(item.body || ''); alert('Copied prompt to clipboard.'); } catch {}
      };
...
document.addEventListener('vg-team-prompts-refresh', __vgPaintTeamPromptsList);
	...
	function attachPortalTooltip(trigger, tipEl){
	  ...
	  const scheduleClose = () => {
	    clearTimeout(hideTimer);
	    hideTimer = setTimeout(() => { tipEl.style.display = 'none'; }, 100);
	  };
	...
	try {
	  const behaveInfos = shadow.querySelectorAll('.behave .tooltip .info') || [];
	  const behaveTips  = shadow.querySelectorAll('.behave .tooltip .tip')  || [];
	  behaveInfos.forEach((btn, i) => {
	    const tip = behaveTips[i];
	    if (btn && tip) { try { attachPortalTooltip(btn, tip); } catch {} }
	  });
	} catch {}
	...
	function vgImpOpen(){ q('#vgImpOverlay').hidden = false; vgImpShowStep(0); }
	function vgImpClose(){ q('#vgImpOverlay').hidden = true; }
	
	q('#cgImport')?.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); vgImpOpen(); });
	q('#vgImpClose')?.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); vgImpClose(); });
	...
	q('#vgImpImport')?.addEventListener('click', async (e) => {
	  ...
	  try {
	    ...
	    const looksTSV  = (firstLine.indexOf('\t') >= 0) && (firstLine.indexOf(',') === -1);
	    if (looksTSV) {
	    ...
	    const idxName =
	      headers.indexOf('prompt name') >= 0 ? headers.indexOf('prompt name') :
	    ...
	    if (idxName === -1 || idxBody === -1) {
	      alert("CSV must include 'Prompt Name' (or Name) and 'Prompt Text' columns.");
	      return;
	    }
	    ...
	    const keyOf = (n, p) => (n.toLowerCase().trim() + '::' + p.toLowerCase().trim());
	    const seen = new Set();
		...
		try {
		  ...
		  existingKeys = new Set(existing.map(g =>
		    (g.name || '').toLowerCase().trim() + '::' + (g.body || '').toLowerCase().trim()
		  ));
		} catch (_) {
	    ...
	    for (let i = 0; i < rows.length; i++) {
	      ...
	      if (!name.trim() || !body.trim()) { skipped++; continue; }
	
	      const k = keyOf(name, body);
	      if (seen.has(k) || existingKeys.has(k)) { skipped++; continue; }
	      seen.add(k);
	      ...
	      if (idxTags >= 0 && r[idxTags]) {
	        const raw = r[idxTags];
	        if (/^\s*\[/.test(raw)) {
	          try { const a = JSON.parse(raw); if (Array.isArray(a)) tags = a.map(x => String(x).trim()).filter(Boolean); }
	          catch { /* ignore; fallback below */ }
	        }
	        if (!tags.length) tags = raw.split(/[,;]+/).map(s => s.trim()).filter(Boolean);
	      }
	      // category
	      const category = (idxCat >= 0 && r[idxCat]) ? r[idxCat].replace(/^"|"$/g,'').trim() : null;
	    ...
	    // Refresh and close
	    try { await cgLoadAndRender(); } catch(_) {}
	    vgImpClose();
	    ...
	    if (skippedCount) {
	      msg += ` ${skippedCount} row${skippedCount===1?'':'s'} were skipped.`;
	...
	q('#vgImpDownloadTpl')?.addEventListener('click', (e) => {
	  e.preventDefault();
	  const tpl =
	`Prompt Name,Prompt Text,Tags,Category
	...
	"Create CI workflow","Create a CI workflow with lint, test, build, and artifact caching.","devops,ci","DevOps"`;
	  ...
	  a.href = url; a.download = 'prompt_import_template.csv';
	  a.click();
    ...
    function __vgCloseSettingsModal() {
      try { window.__VG_MODAL_ACTIVE = false; } catch(_) {}
...
async function cgLoadAndRender() {
  try { await __vgEnsureBGSessionFromSoT(); } catch {}
  ...
  try { await S_set('custom_selected', Array.from(selectedCustomIds)); } catch(_) {}

  // keep the last query visible in the input
  try { const s = q('#mpSearch'); if (s) s.value = __CG_QUERY || ''; } catch {}
  ...
  render();  // update preview
	...
	function renderCustomList() {
	  ...
	  const list = !ql ? __CUSTOMS : __CUSTOMS.filter(item => {
...
tog.onclick = async (e) => {
  ...
  try {
    await __vgEnsureBGSessionFromSoT();
    const r = await sendBG('VG_GUARD_SET_STATUS', { id: item.id, on: nextOn });
  ...
  await cgLoadAndRender();
	    ...
	    btnEdit.onclick = (e) => {
	      ...
	      cgmCreate2.onclick = async () => {
	        ...
	        if (!newBody) { cgmBody.focus(); return; }
...
async function seedStdGuardsIntoQuickAddsOnce() {
  try {
    ...
    const stdIds = lib
      .filter(p => String(p["Subcategory"] || '').toLowerCase() === 'standard guards')
      .map(p => p.id);
    ...
    for (const id of toAdd) { try { await vgQAToggle(id); } catch {} }
  ...
  (async () => {
	...
	idsToShow.forEach((id) => {
	  ...
	  const isFree = (p["Free"] === true) ||
	                 (p["PriceCents"] == null) ||
	                 (p["PriceCents"] === 0) ||
	                 (Array.isArray(p["Labels"]) && p["Labels"].some(t => String(t).toLowerCase() === 'free'));
	  ...
	  name.innerHTML = `
	    ...
	    <span class="muted" style="font-size:12px;">${p["Type"] || 'Prompt'} • ${p["Subcategory"] || 'Library'}</span>
	  `;
	...
	const descs = {
	  ...
	  noref: "Prevents AI from rewriting or reorganizing existing code structure.",
	...
	function applyPreset(id) {
	  ...
	  try { S_set("protections_on", Array.from(active)); } catch(_){}
	}

	// (from OLD content.js) — the live preview & enablement
	function render() {
	  // collect selected custom guard objects by id
	  const selectedCustomGuards = __CUSTOMS.filter(x => selectedCustomIds.has(x.id));
...
if (rows) {
  rows.addEventListener("click", async (e) => {
    ...
    if (nowOn) active.add(id); else active.delete(id);

    const list = Array.from(active);
    const inferred = inferPresetIdFromList(list);
    if (presetSel) presetSel.value = inferred;

    try { await S_set("protections_on", list); await S_set("preset_id", inferred); } catch {}
    try { await saveSettingsRoamingOrPromptLogin({ preset_id: inferred, protections_on: list }); } catch {}
...
(async () => {
  ...
  try {
    const db = await DB_getUserSettings(); // { auto_chat, send_delay_sec } or null
      ...
      __VG_SETTINGS.send_delay_sec = parseInt(db.send_delay_sec || 0, 10) || 0;
      ...
      if (Array.isArray(db.protections_on) && db.protections_on.length) {
        ...
        if (db.preset_id && presetSel) { presetSel.value = db.preset_id; }
  ...
  if (!seededFromDB) {
	...
	__VG_SETTINGS.auto_chat = (auto_chat === "1" || auto_chat === 1 || auto_chat === true);
...
if (!autoChatBtn || !countdownBtn || !secsSel) {
  console.debug('[VG][settings] behavior controls hidden (coming-soon message active)');
} else {
  ...
  try { secsSel.value = String(__VG_SETTINGS.send_delay_sec || 5); } catch {}
  reflectBehavior();
...
presetSel?.addEventListener("change", async (e) => {
  ...
  // DB
  try {
    const ok = await saveSettingsRoamingOrPromptLogin({ preset_id: id, protections_on: list });
  ...
  const { booted, preset_id, protections_on } =
    await S_getMany(["booted", "preset_id", "protections_on"]);
  ...
  if (!booted) {
    ...
    applyPreset("all");
    await S_set("protections_on", Array.from(active));
  } else if (protections_on && Array.isArray(protections_on) && protections_on.length) {
    if (rows) {
      rows.querySelectorAll(".tog").forEach((el) =>
...
saveBtn?.addEventListener("click", async (e) => {
  ...
  try {
    const ok = await saveSettingsRoamingOrPromptLogin({ preset_id: pid, protections_on: list });
  ...
  try {
    document.dispatchEvent(new CustomEvent("vg-standard-guards-updated", { detail: { on: list, preset: pid } }));
  } catch {}
...
(function initTabs(){
  ...
  function showTab(which){
    ...
    try {
      if (which === 'advanced') {
        seedStdGuardsIntoQuickAddsOnce().then(() => { try { renderQuickAddsList(); } catch {} });
      }

      if (which === 'team') {                              // ← NEW: paint team list on view
        __vgPaintTeamPromptsList?.();
      }

      if (which === 'library') {
        const root = shadow.querySelector('section[data-tab="library"] #vg-lib-root');
        if (root) vgRenderMarketplaceGrid(root);
        const btn = shadow.querySelector('#vg-lib-refresh');
        if (btn) btn.onclick = async (e) => {
          e.preventDefault();
          try { await vgRefreshLibraryNow(); } catch {}
          try { await vgLoadOwnershipSets(); } catch {}
          if (root) vgRenderMarketplaceGrid(root);
        };
      }
      ...
      if (which === 'settings') {
        try { __vgSettingsRefresh?.(); __vgSettingsWire?.(); } catch {}
      }

      if (which === 'billing') { renderPlanCardsOnce(shadow); renderBilling?.(); }
  ...
  showTab(defaultTab);
	...
	function billCheckSVG(){
	  ...
	  s.innerHTML = '<path d="M20 6L9 17l-5-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
	  return s;
	}
	function billPlanCard(kind, shadow){
	  ...
	  if (spec.popular) {
	    ...
	    badge.className = 'badge'; badge.textContent = 'Most popular';
	    card.appendChild(badge);
	  }
	
	  const hdr = document.createElement('div'); hdr.className = 'hdr';
	  const name = document.createElement('div'); name.className='name';  name.textContent = spec.title;
	  const price= document.createElement('div'); price.className='price'; price.textContent= spec.price;
	  hdr.append(name, price); card.appendChild(hdr);
	
	  const bl = document.createElement('div'); bl.className='bullets';
	  spec.bullets.forEach(t => {
	    const row = document.createElement('div'); row.className='bullet';
	    const icon = billCheckSVG();
	    const txt  = document.createElement('div'); txt.textContent = t;
	    row.append(icon, txt); bl.appendChild(row);
	  });
	  card.appendChild(bl);

	  if (spec.footnote) {
	    const foot = document.createElement('div'); foot.className='foot'; foot.textContent = spec.footnote;
	    card.appendChild(foot);
	  }
...
async function renderBilling() {
  try {
    ...
    const limit = (typeof summary.limit === 'number' || summary.limit === Infinity)
      ? summary.limit
    ...
    if (usageEl) {
      const guardsStr = (limit === Infinity) ? `${usedGuards} / ∞` : `${usedGuards} / ${limit}`;
      const quickStr  = (limit === Infinity) ? `${usedQuick} / ∞`  : `${usedQuick} / ${limit}`;
	...
	// Buttons
	B('billBuyBasic')?.addEventListener('click', () => billingStartCheckout(PRICE_BASIC));
	B('billBuyPro')?.addEventListener('click',   () => billingStartCheckout(PRICE_PRO));
    ...
    function openTemplateModal(id){
	  ...
	  if (rows) {
 	   try {
 	     const row = Array.from(rows.querySelectorAll("tr"))
 	       .find(tr => tr.querySelector('.preview-btn')?.dataset.guard === id);
    ...
    function hideAiOverlay(removeNode = false) {
      ...
      clearInterval(__aiDotsTimer);
      __aiDotsTimer = null;
      try { aiOverlay.hidden = true; } catch {}
    ...
    const AI_ENHANCE_URL = 'https://auudkltdkakpnmpmddaj.supabase.co/functions/v1/ai-enhance';
...
const res = await fetch(AI_ENHANCE_URL, {
  ...
  body: JSON.stringify({ prompt: txt, name })
    ...
    cgmModal.addEventListener('keydown', (e) => {
      if (e.key !== 'Escape') e.stopPropagation();
    }, true);
...
document.addEventListener('vg-edit-prompt', (ev) => {
  try {
    const d = ev?.detail || {};
