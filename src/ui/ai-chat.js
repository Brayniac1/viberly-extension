...
(() => {
  ...
  async function collectLastMessages(n = 10) {
    ...
    try {
      ...
      if (!nodes.length)
        nodes = Array.from(root.querySelectorAll("p,li,pre,blockquote"));
  ...
  let __resumeResolve;
  const __resumeReady = new Promise((r) => {
    __resumeResolve = r;
  });
  ...
  window.openAiChatModal = async function openAiChatModal() {
    ...
    async function runIntroSummaryInto(uiBody) {
      const intro = addMessage(
        uiBody,
        "vg",
        "Analyzing the last 10 messages…",
        { showMeta: false, minWidth: 260 }
      );
      try {
        ...
        const ans = await sendToBG({
          ...
          site,
          messages: last10.length
            ? last10
            : [{ role: "user", text: "(No messages found on page)" }],
        });
        ...
        intro.setText(
          summary || "I couldn’t get a summary just now. Want me to try again?"
        );
        intro.showMeta();
        ...
        intro.setText(
          "I couldn’t analyze the page messages. You can still tell me what you need."
        );
        intro.showMeta();
      }
      return null;
    ...
    async function startNewSession() {
      try {
        await browser.storage.local.remove("ai_chat_active_session_id");
      } catch {}
      sessionId = null;
      ...
      if (sid) {
        sessionId = sid;
        browser.storage.local.set({ ai_chat_active_session_id: sessionId });
      }
    }
    ...
    let selSnippet = "";
    const hideInsert = () => {
      ui.insertBtn.style.display = "none";
      selSnippet = "";
    };
    const showInsertForSelection = () => {
      const sel = window.getSelection?.();
      if (!sel || sel.isCollapsed) return hideInsert();
      let range;
      try {
      ...
      const left = Math.min(
        Math.max(selRect.right - boxRect.left - ui.insertBtn.offsetWidth, 6),
        ui.box.clientWidth - ui.insertBtn.offsetWidth - 6
      );
      const top = Math.max(
        selRect.top - boxRect.top - ui.insertBtn.offsetHeight - 6,
        6
      );
    ...
    ui.body.addEventListener(
      "keyup",
      (e) => {
        if (e.key === "Escape") hideInsert();
        else showInsertForSelection();
      },
      true
    );
    ui.body.addEventListener(
      ...
      () =>
        setTimeout(() => {
          const sel = window.getSelection?.();
          if (!sel || sel.isCollapsed) hideInsert();
        }, 0),
      true
    );
    ...
    ui.insertBtn.addEventListener("click", () => {
      ...
      try {
        if (typeof window.vgInsertPrompt === "function")
          window.vgInsertPrompt(snippet);
        else if (typeof window.setComposerGuardAndCaret === "function")
          window.setComposerGuardAndCaret(snippet, "");
      } catch {}
      hideInsert();
    });
    ...
    browser.storage.local.get("ai_chat_active_session_id").then(async (r) => {
      sessionId = r?.ai_chat_active_session_id || null;

      try {
        if (sessionId) {
          // RESUME: load transcript, DO NOT summarize again
          const m = await sendToBG({
            type: "AI_CHAT:GET_MESSAGES",
    ...
    ui.btnSummary.addEventListener("click", async () => {
      ...
      try {
        ...
        const summary =
          typeof ans === "string"
            ? ans
            : ans?.summary || ans?.text || "No summary.";
        waiting.setText(summary);
        waiting.showMeta();
    ...
    ui.btnNew?.addEventListener("click", async (e) => {
      e.preventDefault();
      e.stopPropagation();
      const was = ui.btnNew.disabled;
      ui.btnNew.disabled = true;
      try {
        await startNewSession();
      } finally {
        ui.btnNew.disabled = was;
      }
    });

    // send flow
    ui.input.addEventListener("input", () => {
      ui.btnSend.disabled = !/\S/.test(ui.input.value || "");
    });
    ...
    const close = () => {
      try {
        document.getElementById("vg-aichat-open-flag")?.remove();
      } catch {}
      try {
        ui.host.remove();
      } catch {}
