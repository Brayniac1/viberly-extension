  ...
  function openScreenshotOverlay() {
    ...
    function layoutShroud(rect) {
      ...
      Object.assign(shroudRight.style, {
        left: (rect.x + rect.w) + 'px', top: rect.y + 'px',
        width: rightW + 'px',
        height: Math.max(0, rect.h) + 'px',
      });
    ...
    bar.style.font = '13px/1.2 Inter, system-ui, -apple-system, "Segoe UI", Roboto, Arial, sans-serif';
    ...
    Object.assign(box.style, {
      ...
      boxShadow: '0 0 0 1px rgba(124,58,237,.35) inset, 0 20px 60px rgba(0,0,0,.35)',
    ...
    handlesSpec.forEach(([dir, x, y, cur]) => box.appendChild(makeHandle(dir, x, y, cur)));
    ...
    function __vgInsertLinkFallback(el, url) {
      if (IS_CHATGPT) return;   // <- hard block: no text fallback on ChatGPT
      try {
    ...
    function onPointerDownBox(e) {
      ...
      rect0 = readBoxRect();
      try { box.setPointerCapture(pointerId); } catch {}
    ...
    function endAll(e) {
      ...
      if (resizing) {
        resizing = false;
        activeDir = '';
        try { /* handle may hold capture; safe to ignore */ } catch {}
      }
    }
    ...
    function onKey(e) {
      ...
      if (e.key === 'Enter') {
        ...
        Promise.resolve()
          .then(handleCapture)
          .finally(() => { __vgShotCapturing = false; btnCapture.disabled = false; });
        return;
      }
      ...
      const r = readBoxRect();
      let changed = false;
      if (e.key === 'ArrowLeft')  { r.x = r.x - step; changed = true; }
      if (e.key === 'ArrowRight') { r.x = r.x + step; changed = true; }
      if (e.key === 'ArrowUp')    { r.y = r.y - step; changed = true; }
      if (e.key === 'ArrowDown')  { r.y = r.y + step; changed = true; }
      if (changed) {
...
box.style.visibility  = 'hidden';                // <-- add
      ...
      let reply;
      try {
        reply = await chrome.runtime.sendMessage({ type: 'VG_CAPTURE_VISIBLE_TAB' });
      } catch (err) {
      ...
      if (!reply || !reply.ok || !reply.dataUrl) {
        console.error('[VG][screenshot] capture failed:', reply?.error || 'no data');
        return;
      }
      ...
      try {
        ...
        console.log('[VG][screenshot] cropped PNG', { rect: r, blob, dataUrlLen: dataUrl.length });


        // Emit event only when not ChatGPT (prevents “insert twice” via other listeners)
        try {
          if (!IS_CHATGPT) {
            window.dispatchEvent(new CustomEvent('VG_SCREENSHOT_RESULT', {
        ...
        try {
          if (typeof window.vgInsertImage === 'function') {
            ...
            const file = await __vgDataURLtoFile(dataUrl, `screenshot-${Date.now()}.png`);
            const target = __vgFindComposer();
            target?.focus?.();

          // ChatGPT note: paste/drop events often return `false` (preventDefault),
          ...
          if (target) {
            ...
            // ChatGPT accepts paste; skip drop to avoid duplicate image uploads
            if (!IS_CHATGPT) {
...
function closeOverlay() {
  ...
  try { window.removeEventListener('pointerup',   endAll, true); } catch {}
  try { window.removeEventListener('pointercancel', endAll, true); } catch {}
  ...
  document.documentElement.style.overflow = prevOverflow;
    ...
    // Toolbar buttons
    btnCancel.addEventListener('click', (e) => { e.preventDefault(); closeOverlay(); });
  ...
  try { window.openScreenshotOverlay = openScreenshotOverlay; } catch {}

  // Allow BG hotkey → message to open overlay
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg && msg.type === 'VG_SCREENSHOT_OPEN') {
      try { window.openScreenshotOverlay?.(); }
      catch (e) { console.warn('[VG][screenshot] failed to open via hotkey:', e); }
    }
  });
})();
