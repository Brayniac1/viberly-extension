...
function prettyLimit(n){ return (n === Infinity) ? '∞' : String(n); }
function normTier(t){ return String(t || 'free').toLowerCase(); }
function isCGReason(r){ return String(r||'') === 'custom_guard_limit'; }
...
async function startCheckoutByPlan(plan) {
  ...
  try {
    ...
    const url = 'https://auudkltdkakpnmpmddaj.supabase.co/functions/v1/create-checkout-session';
...
function planCard(kind, { locked = false } = {}){
  ...
  const card = el('div', { class: `plan ${kind}` + (spec.popular ? ' popular' : '') });
  ...
  hdr.append(el('div', { class:'name',  text: spec.title }), el('div', { class:'price', text: spec.price }));
  ...
  if (spec.footnote) card.appendChild(el('div', { class: 'foot', text: spec.footnote }));

  const btnLabel = locked
    ? (kind === 'basic' ? 'Current plan' : 'Get Pro')
    : (kind === 'pro'   ? 'Get Pro'      : 'Get Basic');

  const planBtn  = el('button', { class: 'planBtn', type: 'button', text: btnLabel });
  if (locked) {
    ...
    planBtn.addEventListener('click', (e) => {
      e.preventDefault();
      try { window.__VG?.log?.('paywall_click_'+kind); } catch {}
...
async function resolvePlanAndUsage(opts = {}) {
  ...
  let tier = null, used = 0, quick = 0, limit = 1;

  try {
    const resp = await new Promise(res => {
      try { chrome.runtime.sendMessage({ type:'VG_ACCOUNT_SUMMARY' }, (r) => res(r)); }
      catch { res(null); }
    });
    if (resp?.ok && resp.summary) {
      ...
      limit = (resp.summary.limit === Infinity) ? Infinity : Number(resp.summary.limit || 1);
    }
  } catch (e) {
...
const { tier, used, quick, limit, reason } = await resolvePlanAndUsage(opts);
const isCG = isCGReason(reason);
    ...
    const noteHTML  = isCG
      ? `Your current plan <b>${tier.charAt(0).toUpperCase()+tier.slice(1)}</b> allows up to <b>${prettyLimit(limit)}</b> unique custom prompts.`
      : `Your current plan <b>${tier.charAt(0).toUpperCase()+tier.slice(1)}</b> allows up to <b>${prettyLimit(limit)}</b> unique quick adds.`;

    const root = document.body || document.documentElement;
    const host = el('div', { id: 'vg-paywall-fallback' }, []);
    host.style.cssText = 'position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.55)';
    root.appendChild(host);

    const card = el('div', {}, []);
    card.style.cssText = 'width:min(860px,96vw);border-radius:14px;background:#0f1116;color:#e5e7eb;border:1px solid #2a2a33;box-shadow:0 40px 120px rgba(0,0,0,.6);overflow:hidden;font:13px/1.45 Inter,system-ui,-apple-system,"Segoe UI",Roboto,Arial,sans-serif';
    host.appendChild(card);

    const hdr = el('div', {}, []);
    hdr.style.cssText = 'display:flex;align-items:center;gap:10px;padding:14px 16px;border-bottom:1px solid #242634;background:#14151d';
    const ic = el('img', { alt: 'Viberly' }, []);
    ic.src = chrome.runtime.getURL('assets/Viberly-transparent.svg');
    ic.style.cssText = 'width:22px;height:22px';
    const x = el('div', { text: '✕', 'aria-label': 'Close' }, []);
    x.style.cssText = 'margin-left:auto;width:28px;height:28px;display:flex;align-items:center;justify-content:center;border-radius:8px;background:#1f1f26;color:#cbd5e1;cursor:pointer';
    x.onclick = () => { try { host.remove(); } catch {} };
    hdr.append(ic, x);
    card.appendChild(hdr);

    const body = el('div', {}, []);
    body.style.cssText = 'padding:14px 16px;display:flex;flex-direction:column;gap:12px';
    body.appendChild(el('div', { html: `<div style="font-weight:700;font-size:16px">${titleHTML}</div>` }));
    body.appendChild(el('div', { html: noteHTML, style: 'color:#a1a1aa' }));

    const note = el('div', {}, []);
    note.style.cssText = 'border:1px solid #2a2a33;border-radius:10px;background:#0c0e13;padding:10px 12px';
    const usedVal = isCG ? (opts.used ?? 0) : (opts.quick ?? 0);
note.appendChild(el('div', { text: `Usage: ${prettyLimit(usedVal)} / ${prettyLimit(limit)}`, style: 'color:#a1a1aa' }));

    note.appendChild(el('div', { html: 'To continue, choose a plan below.', style: 'margin-top:6px' }));
    body.appendChild(note);
    card.appendChild(body);

    const ftr = el('div', {}, []);
    ftr.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:10px;padding:12px 16px;border-top:1px solid #242634;background:#0f1116';
    const left = el('div', {}, []); left.style.cssText = 'display:flex;gap:10px;align-items:center';
    const right= el('div', {}, []); right.style.cssText = 'display:flex;gap:10px;align-items:center';
    const later= el('button', { text: 'Not now', type: 'button' }, []);
    later.style.cssText = 'padding:10px 14px;border-radius:10px;cursor:pointer;font-weight:600;border:1px solid #2a2a33;background:#1f1f26;color:#e5e7eb';
    later.onclick = () => { try { host.remove(); } catch {} };

    const onBasic = (tier === 'basic');
    const basic = el('button', { text: onBasic ? 'Current plan' : 'Upgrade – Basic ($4.99/mo)', type: 'button' });
    basic.style.cssText = 'padding:10px 14px;border-radius:10px;cursor:pointer;font-weight:600;border:1px solid #2a2a33;background:transparent;color:#e5e7eb';
    if (onBasic) {
      basic.disabled = true;
      basic.style.opacity = '.55';
      basic.style.cursor  = 'not-allowed';
    } else {
      basic.onclick = (e) => { e.preventDefault(); startCheckoutByPlan('basic'); };
    }

    const pro = el('button', { text: 'Upgrade – Pro ($9.99/mo)', type: 'button' });
    pro.style.cssText = 'padding:10px 14px;border-radius:10px;cursor:pointer;font-weight:600;border:0;background:#7c3aed;color:#fff';
    pro.onclick = (e) => { e.preventDefault(); startCheckoutByPlan('pro'); };

    left.appendChild(later); right.append(basic, pro); ftr.append(left, right); card.appendChild(ftr);

    host.addEventListener('click', (ev) => { if (ev.target === host) { try { host.remove(); } catch {} } });
  } catch (e) {
...
export async function show(opts = {}) {
  try {
    ...
    // Clean previous
    try { document.getElementById('vg-paywall-host')?.remove(); } catch {}
    ...
    const modal   = el('div', { class: 'modal', id: 'md', tabindex: '-1', role: 'dialog', 'aria-modal': 'true', 'aria-label': 'Paywall' });

    // Header
    const header  = el('div', { class: 'header' });
    const icon    = el('img', { class: 'icon', id: 'ic', alt: 'Viberly' });
    icon.src = chrome.runtime.getURL('assets/Viberly-transparent.svg');
...
note.appendChild(el('div', { class: 'muted', text: `Usage: ${prettyLimit(usedVal)} / ${prettyLimit(limit)}` }));
...
body.appendChild(note);
    ...
    setTimeout(async () => {
      ...
      const hidden = !r || r.width === 0 || r.height === 0 || getComputedStyle(overlay).display === 'none';
      if (hidden) {
    ...
    try { modal && typeof modal.focus === 'function' && modal.focus(); } catch {}

    sh.addEventListener('keydown', (e) => { if (e.key === 'Escape') { e.preventDefault(); close(); } }, true);
    document.addEventListener('keydown', function esc(e){
      if (e.key === 'Escape'){ close(); document.removeEventListener('keydown', esc, true); }
    }, true);

    try { window.__VG?.log?.('paywall_shown', { reason, tier }); } catch {}
...
// Export + global shim
try { window.VGPaywall = Object.assign(window.VGPaywall || {}, { show }); } catch {}
try { window.VGPaywall || (window.VGPaywall = { show }); } catch {}
export default { show };
