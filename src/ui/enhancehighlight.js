...
function pillStartEnhancing() {
  __enhancing = true;
  try { pill.disabled = true; pill.style.opacity = "0.90"; } catch {}
  // ensure visible even if selection collapsed
  try { if (pill.style.display === "none") pill.style.display = "inline-block"; } catch {}
  let step = 0;
...
function pillStopEnhancing() {
  __enhancing = false;
  try { pill.disabled = false; pill.style.opacity = "1"; } catch {}
  if (__enhDotsTimer) { clearInterval(__enhDotsTimer); __enhDotsTimer = null; }
  ...
  function stripPromptNameBlock(s) {
    ...
    txt = txt
      .replace(/^\s*custom\s+prompt\s+name:\s*.*$/gim, "")
      .trim();
  ...
  function formatEnhancedToHTML(enhanced) {
    ...
    for (const rawBlock of blocks) {
      ...
      if (isList) {
        ...
        out.push('<ul>' + items.map(li => `<li>${escapeHTML(li)}</li>`).join('') + '</ul>');
        continue;
      }
  ...
  function getActiveComposer(selRange) {
    ...
    const dbSel = (window.__VG_DB_PLACEMENT && window.__VG_DB_PLACEMENT.composer_selector) || "";
    if (dbSel) {
      try {
    ...
    let n = (selRange && selRange.commonAncestorContainer) ||
...
if (snap.kind === "ce" && snap.range && typeof snap.range.deleteContents === "function") {
  const r = snap.range;
  ...
  function getSelectionInfo() {
    ...
    const isVisible = rect.bottom >= 0 && rect.right >= 0 && rect.top <= vh && rect.left <= vw;
  ...
  pill.addEventListener("click", async () => {
    ...
    const resp = await new Promise((resolve) => {
      try {
        chrome.runtime.sendMessage(
          { type: "VG_AI_ENHANCE", payload: { text: selText, surface: "composer" } },
          (r) => {
    ...
    if (enhanced.length > 12000) {
      ...
      try { await navigator.clipboard.writeText(enhanced); } catch {}
...
openEnhanceComparePanel({
  ...
  function openEnhanceComparePanel({ original, enhanced, anchorRect, onReplace }) {
  ...
  Object.assign(wrap.style, {
    ...
    function makeHdr(text, variant /* 'orig' | 'enh' */) {
      ...
      if (variant === 'orig') {
        ...
        h.style.cssText = 'font-weight:600;font-style:italic;font-size:12px;color:#aeb6c2;letter-spacing:.2px;';
      } else {
        // Enhanced header: white, upright
        h.style.cssText = 'font-weight:600;font-size:12px;color:#ffffff;letter-spacing:.2px;';
      }
      return h;
    }
    ...
    const makeSep = () => {
      ...
      s.style.cssText = 'height:0;border-top:1px solid rgba(255,255,255,.10);width:90%;margin:8px auto;';
      return s;
    };
    ...
    function makeBlockOriginal(txt) {
      ...
      b.style.cssText = 'white-space:pre-wrap;word-break:break-word;color:#bfc3cd;font-style:italic;font-size:12px;';
      b.textContent = txt || '';
      return b;
    }
    ...
    function makeBlockEnhanced(txt) {
      ...
      b.style.cssText = 'white-space:pre-wrap;word-break:break-word;color:#d7dbe5;';
      b.textContent = txt || '';
      return b;
    }
  ...
  Object.assign(foot.style, {
    ...
    padding:'10px 12px', borderTop:'1px solid #2a2a33', background:'#0f1116'
  });

  const btnCancel = document.createElement('button');
  btnCancel.textContent = 'Cancel';
  btnCancel.style.cssText = 'background:transparent;border:0;color:#aeb6c2;font:600 12px system-ui,-apple-system,Segoe UI,Inter,Roboto,Arial,sans-serif;cursor:pointer;padding:4px 2px;';
  btnCancel.onmouseenter = () => { btnCancel.style.color = '#cfd3dc'; };
  btnCancel.onmouseleave = () => { btnCancel.style.color = '#aeb6c2'; };
  ...
  btnReplace.onmouseenter = () => { btnReplace.style.background = '#7c3aed'; btnReplace.style.borderColor = '#7c3aed'; btnReplace.style.color = '#fff'; };
  btnReplace.onmouseleave = () => { btnReplace.style.background = '#1f1f26'; btnReplace.style.borderColor = '#2a2a33'; btnReplace.style.color = '#e5e7eb'; };
  ...
  function onKey(e){
    ...
    if (e.key === 'Enter')  { try { onReplace?.(); } catch {} close(); }
  }
  ...
  btnReplace.onclick = () => { try { onReplace?.(); } catch {} close(); };
  ...
  function place() {
    ...
    const MARGIN = 6;
    const left = Math.max(MARGIN, Math.min(Math.round(ar.left + ar.width/2) - HALF, innerWidth - 400 - MARGIN));
    const top  = Math.max(12, Math.round(ar.top - (wrap.offsetHeight || 0) - 8));
...
function __showComposerEnhancing(anchorElOrRect) {
  ...
  label.style.cssText = 'font:600 13px Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;margin-bottom:8px;';
  ...
  function place() {
    const r = rectOf(anchorElOrRect) || { left: innerWidth/2 - 140, top: innerHeight/2 - 24, width: 280, height: 48 };
    const w = host.offsetWidth || 280;
    const h = host.offsetHeight || 40;
    const cx = Math.round(r.left + r.width/2);
    const cy = Math.round(r.top + r.height/2);
    const left = Math.max(6, Math.min(cx - Math.round(w/2), innerWidth - w - 6));
    const top  = Math.max(12, Math.min(cy - Math.round(h/2), innerHeight - h - 12));
  ...
  const onVV = () => place();
  try { window.addEventListener('resize', onVV, { passive:true }); } catch {}
  try { window.addEventListener('scroll', onVV, { passive:true }); } catch {}

  return {
    close() {
      try { cancelAnimationFrame(host.__raf); } catch {}
      try { window.removeEventListener('resize', onVV, { passive:true }); } catch {}
      try { window.removeEventListener('scroll', onVV, { passive:true }); } catch {}
...
async function __vgCallEnhance(text, surface) {
  return new Promise((resolve) => {
    try {
      chrome.runtime.sendMessage(
        { type: "VG_AI_ENHANCE", payload: { text: String(text || ""), surface: String(surface || "composer") } },
        (r) => {
          const err = chrome.runtime.lastError;
          if (err) {
            console.warn("[VG/enhance] lastError:", err.message);
...
function __readComposerText() {
  ...
  const dbSel = (window.__VG_DB_PLACEMENT && window.__VG_DB_PLACEMENT.composer_selector) || "";
  let el = null;
  if (dbSel) { try { el = document.querySelector(dbSel) || null; } catch {} }
  if (!el) {
    // Fallbacks: CE/PM → textarea/input
    el = document.activeElement && isComposerElement(document.activeElement)
      ? document.activeElement
      : (document.querySelector('[contenteditable="true"], [role="textbox"][contenteditable="true"]')
...
window.vgEnhanceSelection = window.vgEnhanceSelection || (async () => {
...
window.vgEnhanceComposerAll = window.vgEnhanceComposerAll || (async () => {
  if (__VG_ENH_INFLIGHT) return false;

  const { el, text } = __readComposerText();
  const src = String(text || "").trim();
  if (!el || !src) { toast("Nothing to enhance."); return false; }
  if (src.length > MAX_LEN) { toast("Text too long to enhance (>8000)."); return false; }

  // show centered progress over the composer
  const anchorRect = (el.getBoundingClientRect && el.getBoundingClientRect()) || null;
