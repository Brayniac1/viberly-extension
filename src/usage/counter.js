...
(() => {
  ...
  chrome.runtime.sendMessage(
    { type:'COUNTER_HANDSHAKE', payload:{ host:HOST, path:PATH } },
    (resp) => {
      ...
      if (resp?.ok && resp.enabled) {
        ...
        if (__VG_DEBUG_COUNTER) console.log('[VG][counter] COUNTER ARMED for', HOST, PATH, resp);
      } else {
        document.documentElement.dataset.vgCounter = 'disabled';
        if (__VG_DEBUG_COUNTER) console.log('[VG][counter] COUNTER NOT armed for', HOST, PATH, resp);
      }
    }
  );
...
try {
  chrome.storage.local.get('VG_DEBUG_COUNTER', (o) => {
...
try {
  const url = chrome.runtime.getURL('src/usage/tokenCounter.js');
  import(url)
    .then(m => { __countTokens = m.countTokens; try { window.__VG_TOK_READY = true; } catch {} })
    .catch(() => { /* fallback stays on heuristic if import fails */ });
} catch (e) { /* ignore */ }
...
async function resolveSelectors(host, path) {
  try {
    const res = await chrome.runtime.sendMessage({ type: 'VG_GET_PAGE_PLACEMENTS', host, path });
    const rows = Array.isArray(res?.placements) ? res.placements : [];

    // prefer rows that give us explicit selectors
    const withSelectors = rows.find(r => (r?.composer_selector || r?.send_selector || r?.message_container_selector)) || rows[0] || null;
    ...
    if (__VG_DEBUG_COUNTER) console.log('[VG][counter] selectors resolved:', SELECTORS);
  } catch (e) {
    if (__VG_DEBUG_COUNTER) console.warn('[VG][counter] selector resolve failed; using generic listeners', e);
...
function clampAndFlag(chars, tokens) {
  let flagged = false;
  let c = chars, t = tokens;
  if (c > MAX_DELTA_CHARS) { c = MAX_DELTA_CHARS; flagged = true; }
  if (t > MAX_EVENT_TOKENS) { t = MAX_EVENT_TOKENS; flagged = true; }
  return { chars: c, tokens: t, confidence: flagged ? 'low' : 'estimated_profile' };
}
...
function __classifyReplyText(s) {
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    if (/^\s{2,}/.test(L) || /[{;}()\[\]<>]|=>|const |let |var |function |class |def |return |import |export |#include|<\w+>/.test(L)) {
      codeLikeLines++;
    }
...
{
  const tokens = __countTokens ? __countTokens(val) : Math.ceil(val.length * 0.25);
  __queueUsage({
    direction: 'in',
...
const hookPromptSend = () => {
  ...
  if (SELECTORS.send) {
      ...
      try {
        ...
        if (!val) {
          ...
          if (el) {
            ...
            else if (el.isContentEditable || el.getAttribute?.('contenteditable') === 'true') val = (el.textContent || '').trim();
            else if (el.getAttribute?.('role') === 'textbox') val = (el.value || el.textContent || '').trim();
          }
        }
        if (!val) return;
    ...
    }, true); // capture clicks before frameworks stop propagation
  }
  ...
  document.addEventListener('keydown', (e) => {
    ...
    else if (el && (el.isContentEditable || el.getAttribute?.('contenteditable') === 'true')) val = (el.textContent || '').trim();
    else if (el && (el.getAttribute?.('role') === 'textbox')) val = (el.value || el.textContent || '').trim();
...
function __flushOutputFromAdded() {
  ...
  if (__skipNextOutput) { __skipNextOutput = false; return; }
  ...
  const { chars: safeChars, tokens: safeTokens, confidence } = clampAndFlag(chars, tokens);

  if (__VG_DEBUG_COUNTER) {
    console.log('[VG][counter] out delta', { chars, tokens_raw: tokens, safeChars, safeTokens, confidence });
  }
...
async function __vgStartPhaseA() {
  ...
  if (!ready) {
    console.log('[VG][counter] Phase A skipped: not armed (handshake not ready)');
    return;
  }
...
if (document.readyState === 'complete' || document.readyState === 'interactive') {
  ...
  document.addEventListener('DOMContentLoaded', () => __vgStartPhaseA(), { once: true });
}
