...
const chromeOk = !!(typeof chrome !== 'undefined' && chrome?.storage?.local);

const storage = chromeOk ? {
  getItem: (k) => new Promise((res) => {
    try { chrome.storage.local.get([k], out => res(out?.[k] ?? null)); }
    catch (_) { res(null); }
  }),
  setItem: (k, v) => new Promise((res) => {
    try { chrome.storage.local.set({ [k]: v }, () => res()); }
    catch (_) { res(); }
  }),
  removeItem: (k) => new Promise((res) => {
    try { chrome.storage.local.remove([k], () => res()); }
    catch (_) { res(); }
  })
} : {
...
function waitForSupabase(maxMs = 5000) {
  ...
  return new Promise((resolve, reject) => {
    const tick = () => {
      ...
      if (Date.now() - start > maxMs) return reject(new Error('supabase UMD not loaded'));
      setTimeout(tick, 25);
    };
    tick();
  });
        ...
        if (typeof chrome !== 'undefined' && chrome?.storage?.local) {
          await new Promise((resolve) => {
            chrome.storage.local.get(null, async (all) => {
              try {
                const sbKey = Object.keys(all).find(k => k.startsWith(keyPrefix) && typeof all[k] === 'string');
                if (sbKey) {
                  ...
                  if (cs?.access_token && cs?.refresh_token) {
                    try {
                      ...
                      if (restoredData?.session?.expires_at) {
                        ...
                        if (expiresAt && (expiresAt - now) < 120) {
                          console.log('[VG:data] Restored token expiring soon, refreshing...');
                          try {
                            await db.auth.refreshSession();
                          } catch (e) {
                            console.warn('[VG:data] Restored token refresh failed:', e);
                          }
                        }
                      }
                    } catch (_) {}
              ...
              } catch (e) { console.warn('[VG:data] manual restore failed', e); }
              resolve();
            });
          });
        }
...
function getCategoryForHost(host) {
  const PROGRAMMING = [
    'lovable.dev', 'bolt.new', 'replit.com', 'stackblitz.com', 'codesandbox.io'
  ];
  const CREATIVE = [
...
function getActiveDomain() {
  try { return location.hostname; } catch { return null; }
}

/* ---------- Custom Guards core (vg_guards) ---------- */
async function createGuard({ title, body, variables = [], tags = [], site_category = 'programming', config = {} }) {
  const user = await getUser();
  const { error, data } = await db.from('vg_guards')
    .insert({ user_id: user.id, title, body, variables, tags, site_category, config, status: 'active' })
    .select('*').single();
...
async function updateGuard(guardId, patch) {
  ...
  const { error, data } = await db.from('vg_guards')
    .update({ ...patch })
    .eq('id', guardId).eq('user_id', user.id)
    .select('*').single();
  if (error) throw error;
  return data;
}
...
async function setGuardStatus(guardId, on) {
  const user = await getUser();
  const { error } = await db.from('vg_guards')
...
async function deleteGuard(guardId) {
  const user = await getUser();
  const { error } = await db.from('vg_guards')
    .delete().eq('id', guardId).eq('user_id', user.id);
  if (error) throw error;
  return true;
}
...
async function listGuards({ site_category = 'programming', includeInactive = true } = {}) {
  const user = await getUser();
...
async function upsertSiteSettings(domain, partialSettingsJson) {
  const { data, error } = await db.rpc('vg_upsert_site_settings', {
    p_domain: domain, p_settings: partialSettingsJson
  });
  if (error) throw error;
  return data;
}
async function logEvent(name, payload = {}) {
  ...
  const { error } = await db.rpc('vg_log_event', {
    p_name: name, p_site_domain: domain, p_payload: payload
  });
  if (error) throw error;
}
...
function exposeVG() {
  if (!db) { console.warn('[VG:data] exposeVG(): db not ready yet'); return; }
  ...
  try { window.VG = VG_API; } catch {}
  try { self.VG = VG_API; } catch {}
  try { globalThis.VG = VG_API; } catch {}
