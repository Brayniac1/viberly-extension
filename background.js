...
function dbgDebug(...args) { if (DEBUG) console.debug(...args); }
...
const vgErr   = (...a) => { if (__ok('error')) console.error('[VG]', ...a); };
const vgWarn  = (...a) => { if (__ok('warn'))  console.warn('[VG]',  ...a); };
const vgInfo  = (...a) => { if (__ok('info'))  console.info('[VG]',  ...a); };
const vgDebug = (...a) => { if (__ok('debug')) console.debug('[VG]', ...a); };
...
const VG_SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF1dWRrbHRka2FrcG5tcG1kZGFqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU3MDA3NTYsImV4cCI6MjA3MTI3Njc1Nn0.ukDpH6EXksctzWHMSdakhNaWbgFZ61UqrpvzwTy03ho';
// -----------------
...
const sbStorage = {
  getItem: (k) => new Promise(res => chrome.storage.local.get([k], o => res(o?.[k] ?? null))),
...
async function __vgSetAccessSnapshot(snap) {
  try { await chrome.storage.local.set({ [VG_ACCESS_STORAGE_KEY]: snap }); } catch {}
  return snap;
}
...
const __VG_ALLOWED_WHEN_BLOCKED = new Set([
  'ACCESS_STATUS','ACCESS_RECHECK','AUTH_STATUS','GET_STATUS',
  'AUTH_REDIRECT','SET_SESSION','SIGN_OUT','OPEN_POPUP',
  'VG_DEBUG:SESSION_SNAPSHOT','VG_DEBUG:LOAD_SETTINGS','VG_DEBUG:PROFILE',
  'VG_DEBUG:GUARDS_COUNT','VG_DEBUG:FAVS_COUNT','VG_DEBUG:DUMP_USER_DATA','VG_DEBUG:CONFIG',
...
async function __vgGateIfBlocked(type, sender) {
  try {
    ...
    if (__VG_ALLOWED_WHEN_BLOCKED.has(type) || String(type || '').startsWith('VG_DEBUG:')) {
      return null;
    }
...
async function __vgComputeAccessSnapshot() {
  ...
  try {
    ...
    if (!session?.user?.id) {
      ...
      return __vgSetAccessSnapshot({ blocked: false, team: null, reason: null, last_checked: nowIso });
    }
    ...
    if (!teamId) {
      ...
      if (utype === 'individual' && (ustat === 'past_due' || ustat === 'canceled')) {
        const redirect = `https://viberly.ai/individual/subscription-expired?status=${ustat}`;
        console.debug('[VG][access] individual blocked →', { status: ustat, redirect });
      ...
      console.debug('[VG][access] individual allowed →', { status: ustat || 'unknown' });
      return __vgSetAccessSnapshot({ blocked: false, team: false, reason: null, last_checked: nowIso });
    }
    ...
    const snap = {
      ...
      team_seats_purchased: (Number.isFinite(+team.seats_purchased) && +team.seats_purchased >= 5) ? +team.seats_purchased : 5,
...
async function __bgSaveSession(sess) {
  ...
  try { await chrome.storage.local.set({ [VG_STORAGE_KEY]: VG_SESSION }); } catch {}
}
function __bgSnapshot() {
  const s = VG_SESSION;
  const signedIn = !!(s?.access_token && s?.refresh_token && Number.isFinite(s?.expires_at));
  return {
    signedIn,
...
(async () => {
  ...
  try { await __vgBroadcastAuth(__VG_SIGNED_IN); } catch(error) {
...
async function __vgAdoptTokensInBG({ access_token, refresh_token, expires_at }) {
  await client.auth.setSession({ access_token, refresh_token });
  const { data:{ session } } = await client.auth.getSession();
...
async function __vgBroadcastAuth(signed) {
  try {
    ...
    // de-dupe by id
    const byId = new Map();
    [...allowed, ...active].forEach(t => { if (t?.id) byId.set(t.id, t); });
...
function __vgShouldWaitForSession(host = "") {
  try { host = String(host).toLowerCase(); } catch {}
  return (
...
async function __bgAccountSummary() {
  ...
  try {
    ...
    const prof = await client
      .from('vg_profiles')
      .select('tier, custom_guards_count, quick_adds_count, subscription_status')
      .eq('user_id', uid)
      .single();

    const tier   = (prof?.data?.tier) || 'free';
    const used   = Number.isFinite(prof?.data?.custom_guards_count) ? prof.data.custom_guards_count : 0;
    const quick  = Number.isFinite(prof?.data?.quick_adds_count)    ? prof.data.quick_adds_count    : 0;
...
async function __vgFetchTeamPromptsFresh() {
  ...
  const sel = 'id, title, body, tags, status, updated_at, created_at, team_id, ownership_type';
  ...
  return (data || []).map(r => ({
    ...
    updatedAt: r.updated_at ? Date.parse(r.updated_at)
...
async function __vgGetTeamPromptsCached(force = false) {
  ...
  const fresh = !force && sameUser && __vgTeamPromptsCache.ts &&
...
return (rows || []).map(p => {
  ...
  const avatar =
    Array.isArray(cp) ? (cp[0]?.avatar_url || "") : (cp?.avatar_url || "");
...
async function __aiEnhanceInvoke(payload) {
  ...
  const { data, error } = await client.functions.invoke(AI_ENHANCE_FN, { body: payload });
  if (error) {
    // Try to surface response details if present
    let status, text = '';
    try { status = error?.context?.response?.status; } catch {}
    try { text   = await error?.context?.response?.text(); } catch {}
...
async function __vgCallOpenAI(messages, system){
  ...
  if (!openai_key) throw new Error('Missing OpenAI key in chrome.storage.local.openai_key');
...
async function vbFetchPlacement(host, path) {
  try {
    ...
    const pick = (pool.length ? pool : rows).sort((a, b) => {
...
function __vgPrefixCandidates(path = "/") {
  ...
  const prefs = ["/"];
  let cur = "";
  for (const p of parts) { cur += "/" + p; prefs.push(cur); }
...
async function vbStartPlacementSub(tabId, host, path) {
  ...
  if (prev?.chan) { try { prev.chan.unsubscribe(); } catch {} }
  VB_PLACEMENT_SUBS.delete(tabId);

  const chan = client.channel(`vb_pp_${tabId}_${host}`)
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'vg_page_placements', filter: `host=eq.${host}` },
      async () => {
...
vgDebug('[VB_PUSH]', { tabId, host, path, dx: pick?.dx, dy: pick?.dy, corner: pick?.anchor_corner, strat: pick?.pick_strategy || pick?.strategy });
chrome.tabs.sendMessage(
  tabId,
    ...
    try {
      ...
      chrome.tabs.sendMessage(tabId, { type: 'VG_PING_QM' }, (resp) => {
        ...
        chrome.scripting.executeScript(
          ...
          () => chrome.tabs.sendMessage(tabId, { type: 'VG_PING_QM' }, (r2) => resolve(!!r2?.qm))
        );
      });
    } catch (e) {
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  try {
    ...
    const exp = ei ? (Math.floor(Date.now()/1000) + ei)
    ...
    await __vgAdoptTokensInBG({ access_token: at, refresh_token: rt, expires_at: exp });
    ...
    try {
      const { __vg_post_bridge_redirect_url } =
        await chrome.storage.local.get('__vg_post_bridge_redirect_url');
    ...
    setTimeout(async () => {
      ...
      } catch (_) { /* tab already gone or navigated */ }
    }, CLOSE_DELAY_MS);
...
async function __vgActiveTabPath() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab?.url) return '/';
    try { return new URL(tab.url).pathname || '/'; }
    catch { return '/'; }
  } catch { return '/'; }
}
...
async function __vgBroadcastSiteAccess(host, state /* 'on' | 'off' */) {
  try {
    ...
    for (const t of tabs) {
      try {
        const th = new URL(t.url || '').hostname.toLowerCase().replace(/^www\./, '');
        if (th === h) {
          chrome.tabs.sendMessage(
            t.id,
...
async function __vussoReadOverride(host, path = "/") {
  try {
    ...
    const { data, error } = await client
      ...
      .select('state, snooze_until, dx, dy, pill_size, anchor_corner, placement_updated_at, path_prefix')
      .eq('user_id', uid)
      .eq('host', h)
...
case 'COUNTER_HANDSHAKE': {
  try {
    const host = (msg?.payload?.host || '').toLowerCase().replace(/^www\./,'');
    const path = (typeof msg?.payload?.path === 'string' && msg.payload.path) ? msg.payload.path : '/';
    ...
    return { ok:true, enabled:true, placement: { id: pick.id || null, host, path } };
  } catch (e) {
...
case 'USAGE_TEST_INGEST': {
  try {
    ...
    const { data, error } = await client.functions.invoke('usage-ingest', { body });
...
case 'VG_USAGE_BATCH': {
  try {
    if (!msg || !Array.isArray(msg.events)) return { ok:false, error:'INVALID_BATCH' };
    ...
    if (!session?.access_token) {
      ...
      return { ok:true, received: msg.events.length, persisted: 0, reason: 'NO_SESSION' };
    }
    ...
    return { ok:true, received: msg.events.length, persisted: data?.inserted ?? 0 };
  } catch (e) {
...
case 'VG_SAVE_HIGHLIGHT': {
  try {
    console.debug("[BG/VG_SAVE_HIGHLIGHT] in:", {
      hasMsg: !!msg, hasPayload: !!msg?.payload,
   ...
   // No gating on creation. Monetization happens on insertion only.
    ...
    const restUrl = (client && client.rest && client.rest.url) ? client.rest.url : (VG_SUPABASE_URL + "/rest/v1");
    ...
    let meta = {};
    try { meta = await ef.json(); } catch {}
    console.debug("[BG/VG_SAVE_HIGHLIGHT] EF status:", ef.status, "meta:", meta);

    if (!ef.ok) {
      const errMsg = String(meta?.error || `EDGE_FUNCTION_FAILED_${ef.status}`);
    ...
    const { data: ins, error } = await client.from('vg_guards').insert(row).select('id').limit(1);
    if (error) {
...
case 'VG_AI_ENHANCE': {
  try {
    ...
    if (text.length > 8000) return { ok:false, error:'TEXT_TOO_LONG' };
        ...
        const enhanced = (out && typeof out.enhanced === 'string') ? out.enhanced : '';
...
case 'SET_SESSION': {
  const { access_token, refresh_token, expires_at: expIn, userId, email } = msg || {};
  ...
  try {
    ...
    const fpNew = access_token.slice(0,12) + '.' + refresh_token.slice(0,12) + '.' + exp;
    const fpOld = VG_SESSION
      ? (VG_SESSION.access_token?.slice(0,12) + '.' + VG_SESSION.refresh_token?.slice(0,12) + '.' + VG_SESSION.expires_at)
      : '';
...
case 'AUTH_STATUS': { // legacy boolean for older callers
...
case 'GET_SITE_ACCESS': {
  ...
  try {
    ...
    global_enabled = !!pick;
  } catch { global_enabled = false; }

  // 2) Per-user override (if any)
  const ov = await __vussoReadOverride(host, path); // path-aware override
  const now = Date.now();
  const snoozed = !!(ov?.snooze_until && (new Date(ov.snooze_until).getTime() > now));
...
case 'SET_SITE_ACCESS': {
  ...
  try {
    if (reqState === 'off') {
      await client
        .from('vg_user_site_overrides')
        .upsert({ user_id: uid, host, path_prefix: '/', state: 'off' }, { onConflict: 'user_id,host,path_prefix' });
    } else if (reqState === 'on') {
      // optional support for 'on' to force-enable even when global is OFF
      await client
        .from('vg_user_site_overrides')
        .upsert({ user_id: uid, host, path_prefix: '/', state: 'on' }, { onConflict: 'user_id,host,path_prefix' });
    } else {
  ...
  try { await __vgBroadcastSiteAccess(host, enabled ? 'on' : 'off'); } catch {}
    ...
    case 'VG_CAN_INSERT_CUSTOM': {
      try {
        ...
        try {
          ...
          if (tabId) chrome.tabs.sendMessage(tabId, { type:'VG_PAYWALL_SHOW', payload:{ reason:'custom_guard_limit' } });
        } catch {}
...
// …send VG_PAYWALL_SHOW and return QUICK_ADD_LIMIT
        ...
        try {
          const tabId = sender?.tab?.id;
          if (tabId) chrome.tabs.sendMessage(tabId, { type:'VG_PAYWALL_SHOW', payload:{ reason:'quick_add_limit' } });
        } catch {}
...
case 'VG_LOAD_SETTINGS': {
  ...
  return {
    ...
    data: {
      ...
      send_delay_sec: Number.isFinite(+s.send_delay_sec) ? (+s.send_delay_sec) : 0,
      preset_id: (typeof s.preset_id === 'string' && s.preset_id) ? s.preset_id : null,
      protections_on: Array.isArray(s.protections_on) ? s.protections_on.filter(Boolean) : null,
    }
  };
}
    ...
    case 'VG_SAVE_SETTINGS': {
      ...
      const next = {
        ...
        ...(('send_delay_sec' in patch) ? { send_delay_sec: parseInt(patch.send_delay_sec||0,10)||0 } : {}),
        ...(('preset_id'      in patch) ? { preset_id: (patch.preset_id || null) } : {}),
        ...(('protections_on' in patch) ? { protections_on: Array.isArray(patch.protections_on) ? patch.protections_on : null } : {}),
      };
      ...
      if (updErr || !Array.isArray(upd) || upd.length === 0) {
        ...
        const { error: insErr } = await client
          .from('vg_profiles')
          .upsert({ user_id: session.user.id, settings: next }, { onConflict: 'user_id' });
        ...
        const dataUrl = await new Promise((resolve, reject) => {
          chrome.tabs.captureVisibleTab(
            ...
            (url) => {
              ...
              if (err || !url) return reject(new Error(err?.message || 'capture failed'));
              resolve(url);
            }
          );
        });
...
case 'FETCH_PROMPTS': {
  try {
    const items = await fetchPromptsREST(msg?.filter || { status: 'published' });
...
case 'GET_TEAM_PROMPTS': {
  try {
    ...
    return { ok: true, prompts: rows, hasAny: rows.length > 0, cachedAt: __vgTeamPromptsCache.ts };
  } catch (e) {
    return { ok: false, error: String(e?.message || e) };
  }
}
...
case 'TEAM_CHECKOUT_START': {
  try {
    ...
    if (!session?.user?.id) return { ok:false, error:'NOT_SIGNED_IN' };
    const now = Math.floor(Date.now()/1000);
    if (Number.isFinite(session.expires_at) && (session.expires_at - now) < 30) {
    ...
    const seats    = (seatsMsg >= 5 ? seatsMsg
                      : (Number.isFinite(+snap?.team_seats_purchased) ? +snap.team_seats_purchased : 5));
    const isAnnual = (typeof msg?.isAnnual === 'boolean') ? !!msg.isAnnual : true;
    const getUnitPrice = (n) => (n >= 26 ? 7.99 : (n >= 11 ? 8.99 : 9.99));
    const unitPrice = getUnitPrice(seats);
    vgDebug('Invoking EF create-team-checkout:', { seats, isAnnual, unitPrice });
    ...
    try {
      ...
      return { ok:false, error: `EF_THROWN: ${String(thrown?.message || thrown)}` };
    }

    if (r?.error) {
      // Try to pull status/body from supabase-js; if not available, fallback to raw fetch.
      let status = undefined, bodyText = '';
      try { status = r.error?.context?.response?.status; } catch {}
      try { bodyText = await r.error?.context?.response?.text(); } catch {}

      if (!bodyText) {
        // 🔎 Raw fallback: hit the function endpoint directly to capture exact status+text
        try {
          const fnBase = VG_SUPABASE_URL.replace('.supabase.co', '.functions.supabase.co');
      ...
      console.warn('[BG] EF error detail:', { status, bodyText, err: r.error });
      return { ok:false, error: bodyText || r.error?.message || `EF_STATUS_${status || 'UNKNOWN'}` };
    }

    const url = r?.data?.url || r?.data?.checkout_url;
    if (!url) return { ok:false, error:'NO_CHECKOUT_URL' };

    try { await chrome.tabs.create({ url }); } catch (e) { console.warn('[BG] tabs.create failed:', e); }
...
case 'VG_START_PROMPT_CHECKOUT': {
  try {
    ...
    return { ok:true, checkout_url: out.checkout_url, session_id: out.session_id || null };
  } catch (e) {
...
case 'VG_LOG_QUICK_USE': {
  ...
  vgDebug('[bg] VG_LOG_QUICK_USE in →', { prompt_id, hasSession: !!session?.user?.id });
  if (!session?.user?.id) return { ok:false, error:'NO_SESSION' };

  try {
    const out = await client.rpc('vg_log_quick_use', { _prompt_id: prompt_id });
    vgDebug('[bg] RPC out →', out);
    if (out?.error) {
      const msg = String(out.error.message || '').toLowerCase();
      const isLimit = msg.includes('quick add limit');
      const isDup   = msg.includes('duplicate') || msg.includes('already') || msg.includes('exists');
      if (isDup) return { ok:true, reason:'DUP' };                // ← repeat insert is allowed
      return { ok:false, error: out.error.message, reason: isLimit ? 'LIMIT' : 'ERROR' };
    }
...
case 'VG_LOG_GUARD_USE': {
  ...
  const session = await __vgGetSessionMaybeWait(_host);
  vgDebug('[bg] VG_LOG_GUARD_USE in →', { guard_id, hasSession: !!session?.user?.id });
  if (!session?.user?.id) return { ok:false, error:'NO_SESSION' };

  try {
    const out = await client.rpc('vg_log_guard_use', { _guard_id: guard_id });
    vgDebug('[bg] VG_LOG_GUARD_USE RPC →', out);
    if (out?.error) {
      const msg    = String(out.error.message || '').toLowerCase();
      const isLimit= msg.includes('custom guard limit') || msg.includes('guard limit');
      const isDup  = msg.includes('duplicate') || msg.includes('already') || msg.includes('exists');
      if (isDup) return { ok:true, reason:'DUP' };                // ← repeat insert is allowed
      return { ok:false, error: out.error.message, reason: isLimit ? 'LIMIT' : 'ERROR' };
    }
...
case 'VG_RESET_PILL_POS': {
  try {
    const hostMsg = String(msg?.host || '').toLowerCase().replace(/^www\./,'');
...
case 'VG_QM_TOGGLE': {
  try {
    ...
    const getTabId = async () =>
      sender?.tab?.id ?? (await chrome.tabs.query({ active: true, lastFocusedWindow: true }))[0]?.id;
  ...
  () => void chrome.runtime.lastError
    ...
    case 'BUG_BUSTER:SUMMARIZE': {
      ...
      try {
        const text = await __bbSummarizeViaSupabase(site || 'this site', messages || []);
    ...
    case 'AI_CHAT:CREATE_WITH_SUMMARY': {
      ...
      const j = await __aiChatCall(AI_CREATE_URL, { site, summary_text, title });
    ...
    case 'AI_CHAT:SEND': {
      ...
      const j = await __aiChatCall(AI_SEND_URL, { session_id, site, user_text });
...
case 'VG_GET_PAGE_PLACEMENTS': {
  ...
  const path = (typeof msg?.path === 'string' && msg.path) ? msg.path : (await __vgActiveTabPath());
  ...
  try {
    ...
    let ov = null;
    try { ov = await __vussoReadOverride(host, path); } catch {}
...
vgDebug('[GET_PAGE_PLACEMENTS]', {
  host, path, ov_dx: ov?.dx, ov_dy: ov?.dy, ov_corner: ov?.anchor_corner, rows: rows?.length ?? 0
});
...
let session = await __vgGetSessionMaybeWait(_host);
vgDebug('[VG_LIST_CUSTOM_PROMPTS] session?', !!session?.user?.id, 'host=', _host);

    // 🔧 Enhanced session restoration for custom prompts access
    if (!session?.user?.id) {
      console.log('[BG] No session found for custom prompts, attempting restoration...');
      ...
      if (!session?.user?.id) {
        console.log('[BG] Custom prompts: no session after restoration attempts');
        return { ok: true, items: [] };
      }
    }

    // ⚠️ Only select columns that actually exist on vg_guards
    const sel = 'id, title, body, tags, site_category, status, visibility, updated_at, created_at';
...
// We still gate at insertion time via VG_CAN_INSERT_CUSTOM.
    ...
    const { data, error } = await client
      .from('vg_guards')
      .insert(payload)
      .select('id, title, body, tags, site_category, status, visibility, updated_at, created_at')
      .single();
...
case 'VG_GUARD_UPDATE': {
  try {
    ...
    const patch = {};
    if (typeof msg?.patch?.title === 'string') patch.title = msg.patch.title;
...
case 'VG_GUARD_SET_STATUS': {
  try {
    ...
    const { error } = await client
      .from('vg_guards')
      .update({ status: on ? 'active' : 'inactive', updated_at: new Date().toISOString() })
      .eq('id', id)
      .select('id')
      .single();
...
case 'VG_CREATOR_PROMPTS': {
  try {
    ...
    if (!creatorId && !creatorUid) return { ok:false, error:'missing creator_id' };

    // Only select columns that actually exist
    const sel = 'id,name,type,subcategory,labels,prompt_text,status,creator_id,updated_at,is_paid,price_cents';
    ...
    async function fetchBy(val) {
      const { data, error } = await client
        ...
        .order('updated_at', { ascending:false })
        .limit(500);
      if (error) { console.warn('[VG] prompts fetch error:', error); return []; }
    ...
    if ((!rows || rows.length === 0) && creatorUid) rows = await fetchBy(creatorUid);
...
// === DEBUG: snapshot SW auth state (no side effects)
    case 'VG_DEBUG:SESSION_SNAPSHOT': {
	  let userId = null, hasSession = false, email = null;
	  try {
	    const { data: { session } } = await client.auth.getSession();
	    userId     = session?.user?.id || null;
	    email      = session?.user?.email || null;
...
case 'VG_DEBUG:PROFILE': {
  try {
    ...
    const res = await client
      .from('vg_profiles')
      .select('user_id, display_name, tier, subscription_status, custom_guards_count, updated_at, settings')
      .eq('user_id', userId)
      .single();
...
case 'VG_DEBUG:DUMP_USER_DATA': {
  try {
    ...
    const [guards, favs] = await Promise.all([
      client.from('vg_guards')
        .select('id,name,labels,rank,created_at')
        .eq('user_id', userId)
        .order('created_at', { ascending: false })
        .limit(10),
      client.from('vg_quick_favs')
        .select('prompt_id')
        .eq('user_id', userId)
        .limit(10)
    ]);
    ...
    return {
      ...
      guards: guards?.data || [],
      favs: favs?.data || [],
      guards_error: guards?.error ? String(guards.error.message || guards.error) : null,
      favs_error: favs?.error ? String(favs.error.message || favs.error) : null
    };
  } catch (e) {
    return { ok:false, error:String(e?.message || e) };
  }
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  (async () => {
    try {
       ...
       const _host = (() => {
        try { return new URL(_sender?.url || "").hostname.toLowerCase(); }
        catch { return ""; }
      })();
...
try {
  chrome.tabs.onRemoved.addListener((tabId) => {
    ...
    if (sub?.chan) { try { sub.chan.unsubscribe(); } catch {} }
    VB_PLACEMENT_SUBS?.delete?.(tabId);
  });
} catch {}
