...
// ===== COUNTER · Phase 1 Handshake (whitelist check only) =====
    ...
    case "VG_INTENT_FETCH_GUARDS": {
      try {
        ...
        if (guardError) {
          console.warn("[BG/VG_INTENT_FETCH_GUARDS] query error:", guardError);
          return { ok: false, error: String(guardError.message || guardError) };
        }

        const guards = (Array.isArray(guardRows) ? guardRows : []).map(
          (row) => ({
            id: row.id,
        ...
        const promptColumns =
          "id,name,tag_line,prompt_text,labels,updated_at,created_at,status";
        const {
          data: promptRows,
          error: promptError,
        } = await client
        ...
        if (promptError) {
          console.warn(
            "[BG/VG_INTENT_FETCH_GUARDS] prompts query error:",
            promptError
          );
        }

        const globalPrompts = (Array.isArray(promptRows) ? promptRows : [])
          .map((row) => {
            const body =
            ...
            if (!body) return null;
            let preview =
              typeof row.tag_line === "string" ? row.tag_line.trim() : "";
            if (!preview) {
            ...
            return {
              id: row.id,
              title: row.name || "Prompt",
              preview,
              body,
              tags: Array.isArray(row.labels) ? row.labels : [],
              siteCategory: null,
              status: "active",
              visibility: "public",
              config: null,
        ...
        const combined = [...guards, ...globalPrompts];

        return { ok: true, guards: combined };
      } catch (e) {
